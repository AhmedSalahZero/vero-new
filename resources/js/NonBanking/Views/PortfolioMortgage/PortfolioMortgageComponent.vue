...
const hideTablesDates = ref({
  portfolioMortgageRevenueProjectionByCategoryRef: {},
  fundingTableRef: [],
})
const { yearsFromDates: getYearsFromDates } = useDateFormatter(dates)
const lastMonthIndexInEachYear = ref([])
const { hideOrExpandMyYear, hideOrExpandMyYearWithIndex } = useTableExpender(
...
const allTablesTotals = computed(() => {
  return {
    ...
    portfolioMortgageProjectTotals: Helper.calculateTableTotals(
      lastMonthIndexInEachYear,
      model.value?.portfolioMortgageRevenueProjectionByCategories,
      {
        nestedKey: 'portfolio_mortgage_transactions_projections',
      },
    ),
  }
})

const portfolioMortgageProjectTotals = computed(
  () => allTablesTotals.value.portfolioMortgageProjectTotals,
)
...
const totalLoanAmounts = computed(() => {
  ...
  Object.keys(dates.value).forEach((dateAsIndex) => {
    ...
    totalColumn[dateAsIndex] = 0
    Object.keys(model.value.portfolioMortgageRevenueProjectionByCategories || {}).forEach(
      (index) => {
        let portfolioMortgageRevenueProjectionByCategory =
          model.value.portfolioMortgageRevenueProjectionByCategories[index]
        totalColumn[dateAsIndex] += parseFloat(
          portfolioMortgageRevenueProjectionByCategory.portfolio_mortgage_transactions_projections[
            dateAsIndex
          ],
        )
      },
...
const getModelData = () => {
  ...
  const fetchOldDataUrl = `${baseUrl}/${lang}/${companyId}/non-banking-financial-services/study/${studyId}/portfolio-mortgage-fetch-old-data`
  axios
    .get(fetchOldDataUrl, {
      headers: {
...
watch(
  ...
  (newCategories, oldCategories) => {
    ...
    if (newCategories) {
      // 1️⃣ إضافة المفاتيح الجديدة
      Object.keys(newCategories).forEach((index) => {
        if (!hideTablesDates.value.portfolioMortgageRevenueProjectionByCategoryRef[index]) {
          hideTablesDates.value.portfolioMortgageRevenueProjectionByCategoryRef[index] = []
        }
      })

      // 2️⃣ حذف المفاتيح القديمة (Cleanup) 🧹
      Object.keys(hideTablesDates.value.portfolioMortgageRevenueProjectionByCategoryRef).forEach(
        (key) => {
          if (!newCategories[key]) {
            delete hideTablesDates.value.portfolioMortgageRevenueProjectionByCategoryRef[key]
          }
        },
      )
    }
...
<template>
  ...
  <div v-if="!isLoading">
    ...
    <div
      class="kt-portlet"
      v-for="(
        portfolioMortgageRevenueProjectionByCategory, index
      ) in model.portfolioMortgageRevenueProjectionByCategories"
      :key="index">
      <div class="kt-portlet__body">
          ...
          <div class="col-md-11">
            <div class="d-flex align-items-center">
              <h3 class="font-weight-bold form-label kt-subheader__title small-caps">
                Portfolio Mortgage Revenue Projection - Please Choose Duration
                <div
                  class="d-inline-flex w-200"
                  style="width: 200px">
                  <div class="col-md-12">
                    <Select
                      filter
                      v-model="
                        portfolioMortgageRevenueProjectionByCategory.portfolio_mortgage_duration
                      "
                ...
                [you can use the three dots to repeat within the same year]
              </h3>
            </div>
          </div>
          <div class="col-md-1">
            <div class="d-flex justify-content-end">
              <div
                @click="
                  showAndHide.portfolioMortgageProjects[index] =
                    !showAndHide.portfolioMortgageProjects[index]
                "
        ...
        <div
          v-show="showAndHide.portfolioMortgageProjects[index]"
          class="row mt-4">
              ...
              <thead>
                <tr>
                  ...
                  <template
                    ...
                    <template
                      v-if="
                        !hideTablesDates.portfolioMortgageRevenueProjectionByCategoryRef[
                          index
                        ].includes(Number(dateAsIndex))
                      ">
                      <th
                ...
                <tr
                  ...
                  <td>
                    <div class="d-flex flex-column align-items-start">
                      <input
                        :value="'Portfolio Mortgage Projection'"
                        disabled=""
                  ...
                  <td>
                    <div class="d-flex flex-column min-w-percentage mx-auto align-items-start">
                      <InputNumber
                        v-model="
                          model.portfolioMortgageRevenueProjectionByCategories[index].margin_rate
                        "
                        :min="0"
                        :max="100"
                  ...
                  <template
                    ...
                    <td
                      ...
                      <div class="d-flex flex-column align-items-center">
                        <InputNumber
                          v-model="
                            model.portfolioMortgageRevenueProjectionByCategories[index]
                              .portfolio_mortgage_transactions_projections[dateAsIndex]
                          "
                          :min="0"
                          input-class="text-center"
                          :minFractionDigits="0"
                    ...
                    <td
                      ...
                      <InputNumber
                        v-model="
                          portfolioMortgageProjectTotals.subRowTotals[index]['per_year'][
                            dateAsIndex
                          ]
                        "
                        :min="0"
                  ...
                  <td>
                    <InputNumber
                      v-model="portfolioMortgageProjectTotals.subRowTotals[index]['total']"
                      :min="0"
                      input-class="text-center"
                      :minFractionDigits="0"
                      :maxFractionDigits="0"
        ...
        <div class="d-flex mb-3">
          <div class="col-md-6">
            <input
              @click="addNewItem('portfolioMortgageRevenueProjectionByCategories')"
              type="button"
              class="btn btn-primary btn-sm text-white"
              value="Add New" />

            <button
              @click="deleteRepeaterRow(index, 'portfolioMortgageRevenueProjectionByCategories')"
              type="button"
