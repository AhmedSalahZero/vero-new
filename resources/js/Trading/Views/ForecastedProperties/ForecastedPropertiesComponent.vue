						...
						<div v-if="!isLoading">
							...
							<div class="col-md-12">
								<div class="overflow-scroll">
									<div v-for="(forecastedProperty, index) in model.forecastedTradings" :key="index"
...
const getModelData = () => {
	...
	const fetchOldDataUrl = `${baseUrl}/${lang}/${companyId}/property-managements/study/${studyId}/forecasted-Tradings-fetch-old-data`
	axios
		.get(fetchOldDataUrl, {
			headers: {
...
const addNewForecastedPropertyItem = (type) => {
	const emptyRow = empty_rows.value.forecastedTradings
	return model.value.forecastedTradings.push({ ...emptyRow })
}
const deleteRepeaterRow = (index) => {
	model.value.forecastedTradings.splice(index, 1)
}
