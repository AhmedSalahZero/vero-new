...
// Computed
const currentTabTradings = computed(() => {
	propertyStore.setSelectedType(activeTab.value)
	let filtered = propertyStore.filteredTradings
...
const paginatedTradings = computed(() => {
	const start = (currentPage.value - 1) * perPage.value
	const end = start + perPage.value
	return currentTabTradings.value.slice(start, end)
})

const totalPages = computed(() => {
	return Math.ceil(currentTabTradings.value.length / perPage.value)
})

const TradingsCounts = computed(() => {
	return {
		all: propertyStore.allTradings.filter(p => !p.parent_property_id).length,
		unit: propertyStore.unitTradings.length,
		land: propertyStore.landTradings.length,
		complex: propertyStore.complexTradings.length,
		building: propertyStore.buildingTradings.length,
	}
})
...
const handleViewProperty = (propertyId: number) => {
	...
	// console.log('Available Tradings:', propertyStore.Tradings.length)

	const property = propertyStore.Tradings.find(p => p.id === propertyId)
...
onMounted(() => {
	// Get Tradings from blade template
	const TradingsData = (window as any).TradingsData
	const emptyRows = (window as any).emptyRowsData
	if (TradingsData) {
		propertyStore.setTradings(TradingsData as Property[])
				...
				<ul class="nav nav-tabs nav-tabs-space-lg nav-tabs-line nav-tabs-bold nav-tabs-line-3x nav-tabs-line-brand"
					...
					<li class="nav-item">
						<a class="nav-link" :class="{ active: activeTab === 'all' }"
							...
							<i class="fa fa-th-large"></i> {{ $t('All Tradings') }}
							<span class="badge badge-pill badge-secondary ml-1">{{ TradingsCounts.all }}</span>
						</a>
					</li>
					<li class="nav-item">
						<a class="nav-link" :class="{ active: activeTab === 'unit' }"
							...
							<span class="badge badge-pill badge-primary ml-1">{{ TradingsCounts.unit }}</span>
						</a>
					</li>
					<li class="nav-item">
						<a class="nav-link" :class="{ active: activeTab === 'land' }"
							...
							<span class="badge badge-pill badge-success ml-1">{{ TradingsCounts.land }}</span>
						</a>
					</li>
					<li class="nav-item">
						<a class="nav-link" :class="{ active: activeTab === 'complex' }"
							...
							<span class="badge badge-pill badge-warning ml-1">{{ TradingsCounts.complex }}</span>
						</a>
					</li>
					<li class="nav-item">
						<a class="nav-link" :class="{ active: activeTab === 'building' }"
							...
							<span class="badge badge-pill badge-info ml-1">{{ TradingsCounts.building }}</span>
						</a>
					</li>
				</ul>
				<div class="flex-tabs">
							...
							<div class="tab-content kt-margin-t-20">
								<div class="table-responsive">
									<table class="table table-white repeater-class repeater ">
										...
										<tbody>
											<tr v-if="paginatedTradings.length === 0">
												<td colspan="9" class="text-center py-5">
													<i class="fa fa-inbox fa-3x text-muted mb-3 d-block"></i>
													<p class="text-muted">{{ $t('No Tradings found') }}</p>
												</td>
											</tr>
											<PropertyTableRow :model="property" :currentActiveContract="activeTab"
												v-for="property in paginatedTradings" :key="property.id"
								...
								<div class="kt-pagination kt-pagination--brand" v-if="totalPages > 1">
									...
									<div class="kt-pagination__toolbar">
										<span class="pagination__desc">
											...
											{{ Math.min(currentPage * perPage, currentTabTradings.length) }}
											{{ $t('of') }} {{ currentTabTradings.length }}
										</span>
									</div>
								</div>
							</div>
