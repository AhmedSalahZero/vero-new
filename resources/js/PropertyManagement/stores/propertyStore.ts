...
export const usePropertyStore = defineStore('property', () => {
	...
	const unitProperties = computed(() => 
		properties.value.filter(p => p.nature_id === 'unit' && !p.parent_property_id)
	)
	
	const landProperties = computed(() => 
		properties.value.filter(p => p.nature_id === 'land')
	)
	
	const complexProperties = computed(() => 
		properties.value.filter(p => p.nature_id === 'complex')
	)
	const emptyRows = ref<EmptyRow>({} as EmptyRow);
	const buildingProperties = computed(() => 
		properties.value.filter(p => p.nature_id === 'building')
	)
	...
	const getPropertyById = computed(() => {
		return (id: number) => properties.value.find(p => p.id === id)
	})
	
	// Actions
	function setProperties(newProperties: Property[]) {
