...
export function useProperty() {
	...
	const navigateToCreate = (propertyType: string = 'unit') => {
		...
		window.location.href = `${baseUrl}/${lang}/${companyId}/property-managements/properties/create?type=${propertyType}`
	}
	
	const navigateToEdit = (propertyId: number) => {
		const { baseUrl, companyId, lang } = getBaseUrl()
		window.location.href = `${baseUrl}/${lang}/${companyId}/property-managements/properties/${propertyId}/edit`
	}
	const navigateToPropertyExpense = (propertyId: number) => {
		const { baseUrl, companyId, lang } = getBaseUrl()
		window.location.href = `${baseUrl}/${lang}/${companyId}/property-managements/properties/${propertyId}/property-expenses`
	}
	
	const navigateToContracts = (propertyId: number) => {
		const { baseUrl, companyId, lang } = getBaseUrl()
		window.location.href = `${baseUrl}/${lang}/${companyId}/property-managements/properties/${propertyId}/contracts`
	}
	
	const deleteProperty = async (propertyId: number): Promise<boolean> => {
		const { baseUrl, companyId, lang, csrfToken } = getBaseUrl()
		const deleteUrl = `${baseUrl}/${lang}/${companyId}/property-managements/properties/${propertyId}/destroy`
