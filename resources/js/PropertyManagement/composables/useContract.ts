...
export function useContract() {
	...
	const navigateToCreate = () => {
		...
		window.location.href = `${baseUrl}/${lang}/${companyId}/property-managements/properties/${propertyId}/contracts/create`
	}
	const navigateToProperties = () => {
		const { baseUrl, companyId, lang } = getBaseUrl()
		window.location.href = `${baseUrl}/${lang}/${companyId}/property-managements/properties`
	}	
	
	const navigateToEdit = (contractId: number) => {
		const { baseUrl, companyId, lang } = getBaseUrl()
		const propertyId = window.location.pathname.split('/').slice(-2, -1)[0]
		window.location.href = `${baseUrl}/${lang}/${companyId}/property-managements/properties/${propertyId}/contracts/${contractId}/edit`
	}
	
	const deleteContract = async (contractId: number): Promise<boolean> => {
		const { baseUrl, companyId, lang, csrfToken } = getBaseUrl()
		const propertyId = window.location.pathname.split('/').slice(-2, -1)[0]
		const deleteUrl = `${baseUrl}/${lang}/${companyId}/property-managements/properties/${propertyId}/contracts/${contractId}/destroy`
