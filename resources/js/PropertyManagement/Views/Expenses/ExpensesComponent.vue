<template>
					...
					<div class="kt-portlet__body exclude">
						...
						<div class="form-group row justify-content-center"
							...
							<div class="d-flex align-items-center justify-content-start mr-auto">
								<button v-for="(expenseType, index) in expenseTypes" :key="index"
									@click="currentActiveTab = expenseType.id" :class="[
						...
						<div v-if="!isLoading">
							...
							<div v-if="currentActiveTab == 'fixed_monthly_repeating_amount'" class="col-md-12">
								<div v-for="(typeObject, typeIndex) in [
									{
										id: 'fixed_monthly_repeating_amount',
										name: 'Monthly Fixed Expenses',
									},
								]" :key="typeIndex">
									<div v-if="model[typeObject.id]" class="overflow-scroll">
										<div v-for="(item, index) in model[typeObject.id].sub_items" :key="index"
											class="row main-row-style">
											<div class="col-md-1 max-w-trash">
												<div v-if="index > 0"
													class="d-flex flex-column justify-content-start align-items-start">
													<label style="visibility: hidden">Delete</label>
													<button @click="deleteRepeaterRow(index, typeObject.id)"
														type="button"
											...
											<div class="col-md-2 col">
												...
												<Select filter v-model="item.expense_category"
													:options="expenseCategories"
													@change="updateExpenseNamePerCategories(item)" optionLabel="title"
													optionValue="id" placeholder="" checkmark :highlightOnSelect="false"
													class="w-full md:w-56" />
											</div>
											<div class="col-md-2 col">
												<Label :required="false">Expense Name</Label>
												<Select filter v-model="item.expense_name_id"
													:options="item.filteredExpenseNamesOptions" optionLabel="title"
													optionValue="id" placeholder="" checkmark :highlightOnSelect="false"
													class="w-full md:w-56" />
											</div>
											<div class="col-md-1 col min-w-140">
												<Label :required="false">Start Date</Label>
												<VueDatePicker v-model="item.start_date" month-picker auto-apply
													text-input teleport="body" format="MMM-yyyy"
													:min-date="new Date(studyStartDate)"
													:start-date="new Date(studyStartDate)" :max-date="item.end_date ? new Date(item.end_date.year, item.end_date.month) : null
														"></VueDatePicker>
											</div>
											<div class="col-md-1 col">
												<Label :required="false">Amount</Label>
												<InputNumber v-model="item.amount" :min="0" :minFractionDigits="0"
													:maxFractionDigits="0" suffix=" EGP" fluid />
											</div>
											<div class="col-md-1 col min-w-140">
												<Label :required="false">End Date</Label>
												<VueDatePicker v-model="item.end_date" month-picker auto-apply
													text-input teleport="body" format="MMM-yyyy"
													:start-date="new Date(studyStartDate)" :min-date="item.start_date
											...
											<div class="col-md-1 col">
												...
												<Select @change="showCustomPopup(item)" filter
													v-model="item.payment_terms" :options="paymentTerms"
													optionValue="id" optionLabel="title" placeholder="" checkmark
													:highlightOnSelect="false" class="w-full md:w-56" />
												<div v-if="currentActiveCollectionModal == item"
														...
														<div class="modal-content">
															...
															<div class="modal-body">
																<div class="customize-elements">
																	<table class="table exclude-table">
																		...
																		<tbody>
																			<tr v-for="rowIndex in [0, 1, 2, 3, 4]"
																				:key="rowIndex">
																				<td>
																					<div>
																						<InputNumber
															...
															<div class="modal-footer">
																<button type="button" class="btn btn-primary"
																	@click="closePaymentModel()"> Save </button>
															</div>
														</div>
													</div>
												</div>
											...
											<div class="col-md-1 col">
												...
												<div class="form-group">
													<InputNumber :minFractionDigits="2" :maxFractionDigits="2"
														:step="0.25" :min="0" :max="100" mode="decimal" showButtons
														v-model="item.vat_rate" suffix=" %" fluid />
												</div>
											</div>
											<div class="col-md-1 col">
												...
												<div class="form-group">
													<InputNumber :minFractionDigits="2" :maxFractionDigits="2"
														:step="0.25" :min="0" :max="100" mode="decimal" showButtons
														v-model="item.withhold_tax_rate" suffix=" %" fluid />
												</div>
											</div>
											<IncreaseRateModal :typeObject="typeObject" :item="item" :index="index"
												:increase-years-formatted="increaseYearsFormatted" :modals="modals">
												<div class="col-md-1 col" v-if="increaseYearsFormatted.length">
													<Label :required="false">Increase Rate</Label>
													<button
														@click="modals.increaseRate.currentActive = typeObject.id + '-' + index"
														class="btn btn-primary btn-md text-nowrap" type="button">
										...
										<div class="container mt-4">
											<div class="row">
												<div class="col-md-6" style="width: 94%">
													<input @click="addNewItem(typeObject.id)" data-repeater-create=""
							...
							<div v-if="currentActiveTab == 'percentage_of_sales'" class="col-md-12">
								<div v-for="(typeObject, typeIndex) in [
									{
									...
									<div v-if="model[typeObject.id]" class="overflow-scroll">
										<div v-for="(item, index) in model[typeObject.id].sub_items" :key="index"
											...
											<div class="col-md-2 col">
												<Label :required="false">Expense <br /> Category</Label>
												<Select filter v-model="item.expense_category"
													:options="expenseCategories"
													@change="updateExpenseNamePerCategories(item)" optionLabel="title"
													optionValue="id" placeholder="" checkmark :highlightOnSelect="false"
											...
											<div class="col col-md-2">
												<Label :required="false">Expense <br /> Name</Label>
												<Select filter v-model="item.expense_name_id"
													:options="item.filteredExpenseNamesOptions" optionLabel="title"
													optionValue="id" placeholder="" checkmark :highlightOnSelect="false"
													class="w-full md:w-56" />
											...
											<div class="col col-md-2">
												<Label :required="false">Percentage <br /> Of</Label>
												<Select filter v-model="item.percentage_of" :options="percentageOf"
													optionLabel="title" optionValue="id" placeholder="" checkmark
													:highlightOnSelect="false" class="w-full md:w-56" />
											</div>
											<!-- <div class="col col-md-2">
												<label> Revenue <br /> Streams </label>
												<MultiSelect v-model="item.revenue_stream_type" showClear
													:options="revenueStreams"
													@change="updateRevenueCategoriesBasedOnRevenue(item)"
													optionValue="id" optionLabel="title" filter placeholder=""
													:maxSelectedLabels="50" class="w-full md:w-80" />
											</div>
											<div class="col col-md-2">
												<label> Stream <br /> Category </label>
												<MultiSelect v-model="item.stream_category_ids" showClear
													:options="item.filteredRevenueCategoriesOptions" optionValue="id"
													optionLabel="title" filter placeholder="" :maxSelectedLabels="50"
													class="w-full md:w-80" />
											</div> -->
											<div class="col col-md-1 min-w-140">
												<Label :required="false">Start <br /> Date</Label>
												<VueDatePicker v-model="item.start_date" month-picker auto-apply
													text-input teleport="body" format="MMM-yyyy"
													:min-date="new Date(studyStartDate)"
													:start-date="new Date(studyStartDate)" :max-date="item.end_date ? new Date(item.end_date.year, item.end_date.month) : null
											...
											<div class="col col-md-1 min-w-percentage">
												...
												<div class="form-group">
													<InputNumber :minFractionDigits="2" :maxFractionDigits="2"
														:step="0.25" :min="0" :max="100" mode="decimal" showButtons
														v-model="item.monthly_percentage" suffix=" %" fluid />
												</div>
											</div>
											<div class="col col-md-1 min-w-140">
												<Label :required="false">End <br /> Date</Label>
												<VueDatePicker v-model="item.end_date" month-picker auto-apply
													text-input teleport="body" format="MMM-yyyy"
													:start-date="new Date(studyStartDate)" :min-date="item.start_date
														? new Date(item.start_date.year, item.start_date.month)
											...
											<div class="col col-md-1 min-w-160">
												<Label :required="false">Payment <br /> Terms</Label>
												<Select @change="showCustomPopup(item)" filter
													v-model="item.payment_terms" :options="paymentTerms"
													optionValue="id" optionLabel="title" placeholder="" checkmark
													:highlightOnSelect="false" class="w-full md:w-56" />
												<div v-if="currentActiveCollectionModal == item"
													...
													<div class="modal-dialog modal-sm modal-dialog-centered"
														...
														<div class="modal-content">
															<div class="modal-header">
																<h5 class="modal-title"> Custom <br /> Payment </h5>
																<button type="button" class="close"
																	@click="closePaymentModel()">
																	<span aria-hidden="true">×</span>
																</button>
											...
											<div class="col col-md-1 min-w-percentage">
												<Label :required="false">Vat <br /> Rate</Label>
												<div class="form-group">
													<InputNumber :minFractionDigits="2" :maxFractionDigits="2"
														:step="0.25" :min="0" :max="100" mode="decimal" showButtons
														v-model="item.vat_rate" suffix=" %" fluid />
											...
											<div class="col col-md-1 min-w-percentage">
												<Label :required="false">Withhold <br /> Rate</Label>
												<div class="form-group">
													<InputNumber :minFractionDigits="2" :maxFractionDigits="2"
														:step="0.25" :min="0" :max="100" mode="decimal" showButtons
														v-model="item.withhold_tax_rate" suffix=" %" fluid />
							...
							<div v-if="currentActiveTab == 'cost_per_unit'" class="col-md-12">
								<div v-for="(typeObject, typeIndex) in [
									{
										id: 'cost_per_unit',
										name: 'Cost Per Unit',
									...
									<div v-if="model[typeObject.id]" class="overflow-scroll">
										<div v-for="(item, index) in model[typeObject.id].sub_items" :key="index"
											...
											<div class="col-md-1 col">
												<Label :required="false">Cost <br /> Per Contract </Label>
												<InputNumber v-model="item.monthly_cost_of_unit" :min="0"
													:minFractionDigits="0" :maxFractionDigits="0" suffix=" EGP" fluid />
											</div>
											<div class="col col-md-1 min-w-140">
												<Label :required="false">End <br /> Date</Label>
												<VueDatePicker v-model="item.end_date" month-picker auto-apply
													teleport="body" text-input format="MMM-yyyy"
													:start-date="new Date(studyStartDate)" :min-date="item.start_date
														? new Date(item.start_date.year, item.start_date.month)
														: null
														"></VueDatePicker>
											...
											<IncreaseRateModal :typeObject="typeObject" :item="item" :index="index"
												:increase-years-formatted="increaseYearsFormatted" :modals="modals">
												<div class="col-md-1 col" v-if="increaseYearsFormatted.length">
													<Label :required="false">Increase <br /> Rate</Label>
													<button
														@click="modals.increaseRate.currentActive = typeObject.id + '-' + index"
														class="btn btn-primary btn-md text-nowrap" type="button">
														Increase Rate </button>
							...
							<div v-if="currentActiveTab == 'one_time_expense'" class="col-md-12">
								<div v-for="(typeObject, typeIndex) in [
									{
										id: 'one_time_expense',
									...
									<div v-if="model[typeObject.id]" class="overflow-scroll">
										<div v-for="(item, index) in model[typeObject.id].sub_items" :key="index"
											...
											<div class="max-w-trash col-md-1">
												<div v-if="index > 0"
													class="d-flex flex-column justify-content-start align-items-start">
													<label style="visibility: hidden">Delete <br />
														<span>a</span>
													</label>
													<button @click="deleteRepeaterRow(index, typeObject.id)"
														type="button"
											...
											<div class="col-md-1 col">
												<Label :required="false">Amount <br />
													<span style="visibility: hidden">a</span>
												</Label>
												<InputNumber v-model="item.amount" :min="0" :minFractionDigits="0"
													:maxFractionDigits="0" suffix=" EGP" fluid />
											...
											<div class="col-md-1 col">
												<Label :required="false">Amortization <br /> Months</Label>
												<InputNumber v-model="item.amortization_months" :min="1"
													:minFractionDigits="0" :maxFractionDigits="0" fluid />
											</div>
											<div class="col-md-1 col">
												<Label :required="false">Payment <br /> Terms</Label>
												<Select @change="showCustomPopup(item)" filter
							...
							<div v-if="currentActiveTab == 'expense_per_employee'" class="col-md-12">
								<div v-for="(typeObject, typeIndex) in [
									{
									...
									<div v-if="model[typeObject.id]" class="overflow-scroll">
										<div v-for="(item, index) in model[typeObject.id].sub_items" :key="index"
											...
											<div class="col col-md-2">
												...
												<MultiSelect v-model="item.department_ids" showClear
													:options="departments"
													@change="updatePositionsBasedOnDepartments(item)" optionValue="id"
													optionLabel="title" filter placeholder="" :maxSelectedLabels="50"
													class="w-full md:w-80" />
											</div>
											<div class="col col-md-2">
	...
	{{ item.position_ids }}
                        ...
                        > -->
												<label> Employee <br /> Position </label>
												<MultiSelect v-model="item.position_ids" showClear
													:options="item.filteredPositionsOptions" optionValue="id"
													optionLabel="title" filter placeholder="" :maxSelectedLabels="50"
													class="w-full md:w-80" />
											</div>
											<div class="col col-md-1 min-w-140">
											...
											<div class="col-md-1 col">
												<Label :required="false">Monthly Cost <br /> Per Unit </Label>
												<InputNumber v-model="item.monthly_cost_of_unit" :min="0"
													:minFractionDigits="0" :maxFractionDigits="0" suffix=" EGP" fluid />
											</div>
											<div class="col col-md-1 min-w-140">
...
const percentageOf = Helper.getPropertiesPercentageOf()
const currentActiveTab = ref('fixed_monthly_repeating_amount')
const expenseTypes = ref(Helper.getExpenseTypesForProperties())
...
const updateRevenueCategoriesBasedOnRevenue = (item) => {
	...
	item.revenue_stream_type.forEach((revenueStream) => {
		console.log(revenueCategoriesPerRevenue)
...
const getModelData = () => {
	...
	const fetchOldDataUrl = `${baseUrl}/${lang}/${companyId}/property-managements/study/${studyId}/expenses-fetch-old-data`
	axios
		.get(fetchOldDataUrl, {
			headers: {
