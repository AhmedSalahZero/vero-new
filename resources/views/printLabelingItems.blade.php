...
<body>
    ...
    <div style="width:{{ $width }}cm;height:{{ $height }}cm;padding-top:{{ $paddingTop }}cm;padding-bottom:{{ $paddingTop }}cm;padding-right:{{ $paddingLeft }}cm;padding-left:{{ $paddingLeft }}cm;margin-bottom:{{ $marginBottom }}cm;" class="label-elemenet label-border overflow-hidden">
        <div class="code-item">
            <img style="width:{{ 0.5*$height }}cm;height:{{ 0.5*$height }}cm;@if(!$company->labeling_use_client_logo)margin:auto;@endif" src="data:image/png;base64,{!! base64_encode(\QrCode::format('png')->size(300)->generate(qrcodeSpacing($item->getCode($index+1)))) !!}" alt="barcode" />
