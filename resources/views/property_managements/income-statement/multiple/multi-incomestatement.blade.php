                ...
                <div class="kt-portlet kt-portlet--mobile">
                    ...
                    <x-tables.repeater-table :tableClasses="'table-condensed fixed-column-table table-row-spacing income-class-table'" :removeActionBtn="true" :removeRepeater="true" :initialJs="false" :repeater-with-select2="true" :canAddNewItem="false" :parentClass="'js-remove-hidden scrollable-table'" :hide-add-btn="true" :tableName="''" :repeaterId="''" :relationName="'food'" :isRepeater="$isRepeater=!(isset($removeRepeater) && $removeRepeater)">
                        ...
                        <x-slot name="trs">
                            ...
                            @include('property_managements.income-statement.multiple.main_rows',['hasSubItems'=>true,'hasParent'=>false])
                            ...
                            @endphp
                            @include('property_managements.income-statement.multiple.main_rows',['currentTableData'=>$currentMainRows,'study'=>$study,'hasSubItems'=>$hasSubItems,'hasParent'=>true])
                            @foreach($subItems as $subItemId => $subItemArr)
                            @include('property_managements.income-statement.multiple.sub_items')
                            @endforeach
							@if($tableIndex == 0 || $tableIndex == 1) 
                            @include('property_managements.income-statement.multiple.sub_items',['subItemArr'=>[],'enabled'=>true,'subItemId'=>__('Adjusted')])
							@endif
