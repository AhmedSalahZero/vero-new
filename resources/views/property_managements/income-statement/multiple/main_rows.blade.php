...
@endif 
 ...
 data-repeater-style>
    <td class="fixed-column">
        @if($hasSubItems)
        <a href="#" class="btn @if($hasParent) ml-4 @endif btn-1-bg btn-sm btn-brand add-btn-class text-center @if($hasParent) add-btn-js @else add-btn-parent-js @endif">
            <i class="fas fa-angle-double-down expand-icon exclude-icon"></i>
        </a>
        @endif
    </td>
    <td class="fixed-column">
        <div class="d-flex @if(!$hasParent) ml--2 @endif align-items-center justify-content-center flex-column name-max-width-class" style="gap:10px">
            @php
            $currentIndex = 0 ;
            @endphp
