 <form method="post" action="{{ isset($model) ? route('update.property.management.foreign.exchange.rate',['company'=>$company->id, 'foreignExchangeRate'=>$model->id]) :route('store.property.management.foreign.exchange.rate',['company'=>$company->id ]) }}" class="kt-form kt-form--label-right">
                            @csrf
                            @if(isset($model))
                            @method('patch')
                            @endif
