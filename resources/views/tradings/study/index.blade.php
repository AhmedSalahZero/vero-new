        ...
        <div class="kt-portlet__head-toolbar justify-content-between flex-grow-1">
            ...
            <div class="multi-flex-tabs">
                ...
                <div class="flex-tabs mt-2">
                    <a href="{{ route('view.trading.foreign.exchange.rate',['company'=>$company->id]) }}" class="btn w-200px new-record-class new-study-item rounded btn-icon-sm align-self-center">
                        <i class="fas fa-plus white-icon exclude-icon"></i>
                        {{ __('Foreign Exchange Rate') }}
                    </a>

                    <a href="{{ route('trading.create.study',['company'=>$company->id,'is_business_plan'=>0]) }}" class="btn 
                    ...
                    {{-- <a href="{{ route('trading.view.properties',['company'=>$company->id]) }}" class="btn w-200px new-record-class new-study-item rounded btn-icon-sm align-self-center">
                        <i class="fas fa-plus white-icon exclude-icon"></i>
                        {{ __('Properties') }}
                    </a> --}}
                    ...
                    <a href="{{ route('trading.view.departments',['company'=>$company->id]) }}" class="btn w-200px new-record-class new-study-item rounded btn-icon-sm align-self-center">
                        <i class="fas fa-plus white-icon exclude-icon"></i>
                        {{ __('Manpower Structure') }}
                    </a>

                    <a href="{{ route('trading.view.expense.names',['company'=>$company->id]) }}" class="btn w-200px new-record-class new-study-item rounded btn-icon-sm align-self-center">
                        <i class="fas fa-plus white-icon exclude-icon"></i>
                        {{ __('Cost & Expenses') }}
                    </a>
					 <a href="{{ route('trading.view.tenants',['company'=>$company->id]) }}" class="btn w-200px new-record-class new-study-item rounded btn-icon-sm align-self-center">
                        <i class="fas fa-plus white-icon exclude-icon"></i>
                        {{ __('Tenants') }}
                    </a>
                    <a href="{{ route('trading.view.fixed.asset.names',['company'=>$company->id]) }}" class="btn w-200px new-record-class new-study-item rounded btn-icon-sm align-self-center">
                        <i class="fas fa-plus white-icon exclude-icon"></i>
                        {{ __('Fixed Assets') }}
                    </a>
                </div>
                    ...
                    <x-tables.repeater-table :removeActionBtn="true" :removeRepeater="true" :initialJs="false" :repeater-with-select2="true" :canAddNewItem="false" :parentClass="'js-remove-hidden'" :hide-add-btn="true" :tableName="''" :repeaterId="''" :relationName="'food'" :isRepeater="$isRepeater=!(isset($removeRepeater) && $removeRepeater)">
                        ...
                        <x-slot name="trs">
                            ...
                            <tr data-repeat-formatting-decimals="0" data-repeater-style>
                                ...
                                <td>
                                    <div class="d-flex align-items-center flex-column " style="gap:10px;">
                                        <div class="d-flex mr-auto" style="gap:10px;">
                                            <a href="{{ route('view.trading.forecast.income.statement',['company'=>$company->id,'study'=>$model->id]) }}" class="btn btn-md-width btn-1-bg btn-sm btn-brand btn-pill">{{ __('Income Statement') }}</a>
                                            <a href="{{ route('trading.balance.sheet.result',['company'=>$company->id,'study'=>$model->id]) }}" class="btn btn-md-width btn-2-bg btn-sm btn-brand btn-pill">{{ __('Balance Sheet') }}</a>
                                        </div>
                                        <div class="d-flex mr-auto" style="gap:10px;">
                                            <a href="{{ route('trading.cash.in.out.flow.result',['company'=>$company->id,'study'=>$model->id]) }}" class="btn btn-md-width btn-3-bg btn-sm btn-brand btn-pill">{{ __('Cash Flow') }}</a>
                                ...
                                <td>
                                    <div class="d-flex mr-auto" style="gap:10px;">
                                        <a href="{{ route('trading.view.results.dashboard',['company'=>$company->id,'study'=>$model->id]) }}" class="btn btn-sm-width btn-1-bg btn-sm btn-brand btn-pill">{{ __('Result') }}</a>
                                        <a href="{{ route('view.trading.valuation',['company'=>$company->id,'study'=>$model->id]) }}" class="btn btn-sm-width btn-2-bg btn-sm btn-brand btn-pill">{{ __('Valuation') }}</a>
                                    </div>
                                </td>
                                <td class="kt-datatable__cell--left kt-datatable__cell " data-field="Actions" data-autohide-disabled="false">
                                    <span style="overflow: visible; position: relative; width: 110px;">
                                        <a type="button" class="btn btn-secondary btn-outline-hover-brand btn-icon edit-btn-class" title="{{ __('Edit') }}" href="{{ route('trading.edit.study',['company'=>$company->id,'study'=>$model->id]) }}"><i class="fa fa-pen-alt exclude-icon default-icon-color"></i></a>
                                        ...
                                        <div class="modal fade" id="copy{{ $model->id }}" tabindex="-1" role="dialog" aria-labelledby="copyModalLabel{{ $model->id }}" aria-hidden="true" style="color: black">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <form action="{{ route('trading.copy.study', ['study'=>$model->id,'company'=>$company->id]) }}" id="form{{ $model->id }}" method="POST">
                                        ...
                                        <div class="modal fade" id="delete-study-{{ $model->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered" role="document">
                                                <div class="modal-content">
                                                    <form action="{{ route('trading.study.destroy',['company'=>$company->id,'study'=>$model->id ]) }}" method="post">
                                                        @csrf
                                                        @method('delete')
                                                        <div class="modal-header">
                        ...
                        <x-slot name="trs">
                            ...
                            <tr data-repeat-formatting-decimals="0" data-repeater-style>
                                ...
                                <td>
                                    <div class="d-flex align-items-center flex-column " style="gap:10px;">
                                        <div class="d-flex mr-auto" style="gap:10px;">
                                            <a href="{{ route('view.trading.forecast.income.statement',['company'=>$company->id,'study'=>$model->id]) }}" class="btn btn-sm-width btn-1-bg btn-sm btn-brand btn-pill">{{ __('Forecast') }}</a>
                                ...
                                <td>
                                    <div class="d-flex mr-auto" style="gap:10px;">
                                        <a href="{{ route('trading.balance.sheet.result',['company'=>$company->id,'study'=>$model->id]) }}" class="btn btn-sm-width btn-1-bg btn-sm btn-brand btn-pill">{{ __('Forecast') }}</a>
                                        <a href="#" class="btn btn-sm-width btn-2-bg btn-sm btn-brand btn-pill">{{ __('Actual') }}</a>
                                    </div>
                                </td>
                                <td>
                                    <div class="d-flex mr-auto" style="gap:10px;">
                                        <a href="{{ route('trading.cash.in.out.flow.result',['company'=>$company->id,'study'=>$model->id]) }}" class="btn btn-sm-width btn-1-bg btn-sm btn-brand btn-pill">{{ __('Forecast') }}</a>
                                        <a href="#" class="btn btn-sm-width btn-2-bg btn-sm btn-brand btn-pill">{{ __('Actual') }}</a>
                                    </div>
                                ...
                                <td>
                                    <div class="d-flex mr-auto" style="gap:10px;">
                                        <a href="{{ route('trading.view.results.dashboard',['company'=>$company->id,'study'=>$model->id]) }}" class="btn btn-sm-width btn-1-bg btn-sm btn-brand btn-pill">{{ __('Forecast') }}</a>
                                        <a href="#" class="btn btn-sm-width btn-2-bg btn-sm btn-brand btn-pill">{{ __('Actual') }}</a>
                                    </div>
                                </td>
                                <td class="kt-datatable__cell--left kt-datatable__cell " data-field="Actions" data-autohide-disabled="false">
                                    <span style="overflow: visible; position: relative; width: 110px;">
                                        ...
                                        {{-- <a type="button" class="btn btn-secondary btn-outline-hover-brand btn-icon edit-btn-class" title="Edit" href="{{ route('trading.edit.study',['company'=>$company->id,'study'=>$model->id]) }}"><i class="fa fa-pen-alt exclude-icon default-icon-color"></i></a> --}}
                                        {{-- @endif  --}}
