<div class="modal fade inner-modal-class" id="edit-rates-{{ $rate->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                    <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
                                        <div class="modal-content">
                                            <form action="{{ route('overdraft-against-commercial-paper-edit-rates',['company'=>$company->id,'financialInstitution'=>$financialInstitution->id,'rate'=>$rate->id ]) }}" method="post">
                                                @csrf
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <button data-url="{{  route('overdraft-against-commercial-paper-edit-rates',['company'=>$company->id,'financialInstitution'=>$financialInstitution->id,'rate'=>$rate->id]) }}" type="submit" class="btn btn-primary submit-form-btn">{{ __('Confirm') }}</button>
                                                </div>
