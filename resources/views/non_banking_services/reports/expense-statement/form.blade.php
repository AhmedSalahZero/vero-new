...
<div class="row">
    <div class="col-md-12">
        ...
        <!--begin::Form-->
        <form class="kt-form kt-form--label-right" method="post" action="{{ route('non.banking.result.expense.statement.reports',['company'=>$company->id,'study'=>$study->id ]) }}" enctype="multipart/form-data">
            @csrf
...
<script>
	$(document).on('change','select#expense-category-id',function(){
		const val = $(this).val();
		$.ajax({
			url:"{{ route('non.banking.get.expense.name.for.category',['company'=>$company->id , 'study'=>$study->id]) }}",
