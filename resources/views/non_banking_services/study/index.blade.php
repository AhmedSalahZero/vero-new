...
@endif
                ...
                <div class="flex-tabs">
                    ...
                    <a href="{{ route('non.banking.view.departments',['company'=>$company->id]) }}" class="btn w-200px new-record-class new-study-item rounded btn-icon-sm align-self-center">
                        <i class="fas fa-plus white-icon exclude-icon"></i>
                        {{ __('Manpower Structure') }}
                    </a>
                    <a href="{{ route('non.banking.view.expense.names',['company'=>$company->id]) }}" class="btn w-200px new-record-class new-study-item rounded btn-icon-sm align-self-center">
                        <i class="fas fa-plus white-icon exclude-icon"></i>
                        {{ __('Cost & Expenses') }}
                    </a>
                    <a href="{{ route('non.banking.view.fixed.asset.names',['company'=>$company->id]) }}" class="btn w-200px new-record-class new-study-item rounded btn-icon-sm align-self-center">
                        <i class="fas fa-plus white-icon exclude-icon"></i>
                        {{ __('Fixed Assets') }}
                    </a>
                </div>
                            ...
                            <tr data-repeat-formatting-decimals="0" data-repeater-style>
                                ...
                                <td>
                                    <div class="d-flex align-items-center flex-column " style="gap:10px;">
                                        ...
                                        <div class="d-flex mr-auto" style="gap:10px;">
                                            <a href="{{ route('non.banking.cash.in.out.flow.result',['company'=>$company->id,'study'=>$model->id]) }}" class="btn btn-md-width btn-3-bg btn-sm btn-brand btn-pill">{{ __('Cash Flow') }}</a>
                                ...
                                <td>
                                    <div class="d-flex mr-auto" style="gap:10px;">
                                        <a href="{{ route('non.banking.view.results.dashboard',['company'=>$company->id,'study'=>$model->id]) }}" class="btn btn-sm-width btn-1-bg btn-sm btn-brand btn-pill">{{ __('Result') }}</a>
                                    ...
                                    <span style="overflow: visible; position: relative; width: 110px;">
                                        ...
                                        <div class="modal fade" id="copy{{ $model->id }}" tabindex="-1" role="dialog" aria-labelledby="copyModalLabel{{ $model->id }}" aria-hidden="true" style="color: black">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <form action="{{ route('non.banking.copy.study', ['study'=>$model->id,'company'=>$company->id]) }}" id="form{{ $model->id }}" method="POST">
                                                        <div class="modal-body">
                                        ...
                                        <div class="modal fade" id="delete-study-{{ $model->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered" role="document">
                                                <div class="modal-content">
                                                    <form action="{{ route('non.banking.study.destroy',['company'=>$company->id,'study'=>$model->id ]) }}" method="post">
                                                        @csrf
                                                        @method('delete')
                                                        <div class="modal-header">
                    ...
                    <x-tables.repeater-table :removeActionBtn="true" :removeRepeater="true" :initialJs="false" :repeater-with-select2="true" :canAddNewItem="false" :parentClass="'js-remove-hidden'" :hide-add-btn="true" :tableName="''" :repeaterId="''" :relationName="'food'" :isRepeater="$isRepeater=!(isset($removeRepeater) && $removeRepeater)">
                        ...
                        <x-slot name="trs">
                            ...
                            <tr data-repeat-formatting-decimals="0" data-repeater-style>
                                ...
                                <td>
                                    <div class="d-flex mr-auto" style="gap:10px;">
                                        <a href="{{ route('non.banking.balance.sheet.result',['company'=>$company->id,'study'=>$model->id]) }}" class="btn btn-sm-width btn-1-bg btn-sm btn-brand btn-pill">{{ __('Forecast') }}</a>
                                        <a href="#" class="btn btn-sm-width btn-2-bg btn-sm btn-brand btn-pill">{{ __('Actual') }}</a>
                                    </div>
                                </td>
                                <td>
                                    <div class="d-flex mr-auto" style="gap:10px;">
                                        <a href="{{ route('non.banking.cash.in.out.flow.result',['company'=>$company->id,'study'=>$model->id]) }}" class="btn btn-sm-width btn-1-bg btn-sm btn-brand btn-pill">{{ __('Forecast') }}</a>
                                        <a href="#" class="btn btn-sm-width btn-2-bg btn-sm btn-brand btn-pill">{{ __('Actual') }}</a>
                                    </div>
                                ...
                                <td>
                                    <div class="d-flex mr-auto" style="gap:10px;">
                                        <a href="{{ route('non.banking.view.results.dashboard',['company'=>$company->id,'study'=>$model->id]) }}" class="btn btn-sm-width btn-1-bg btn-sm btn-brand btn-pill">{{ __('Forecast') }}</a>
                                        <a href="#" class="btn btn-sm-width btn-2-bg btn-sm btn-brand btn-pill">{{ __('Actual') }}</a>
                                    </div>
                                </td>
                                <td class="kt-datatable__cell--left kt-datatable__cell " data-field="Actions" data-autohide-disabled="false">
