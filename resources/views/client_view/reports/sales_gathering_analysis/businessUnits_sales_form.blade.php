...
@section('css')
    ...
    @endsection
@section('sub-header')
    {{__('Business Units Sales Analysis')}}
@endsection
@section('content')
<div class="row">
    <div class="col-md-12">
        ...
        <!--begin::Form-->
        <form class="kt-form kt-form--label-right" method="POST" action={{ route('businessUnits.sales.analysis.result',$company) }}   enctype="multipart/form-data">
            @csrf
            <div class="kt-portlet">
                <?php $businessUnits =  App\Models\SalesGathering::company()->whereNotNull('business_unit')->where('business_unit','!=','')->groupBy('business_unit')->selectRaw('business_unit')->get()->pluck('business_unit')->toArray();
                ...
                <div class="kt-portlet__body">
                    <div class="form-group row">
						...
						<div class="col-md-4">
                            <label>{{__('Select Business Units')}} 
                            ...
                            <div class="kt-input-icon">
                                <div id="append-main-select" class="input-group date">
                                    <select data-live-search="true" data-actions-box="true" data-max-options="{{ maxOptionsForOneSelector() }}"  name="businessUnits[]"  required class="select2-select form-control kt-bootstrap-select kt_bootstrap_select select2-select"  multiple>
...
<script>
    $(document).on('change', '[name="start_date"],[name="end_date"]', function() {
        ...
        wto = setTimeout(() => {
            var branches = ['all'];
            var type_of_data = "business_unit";
            var getColumnName = 'business_unit';
    ...
    function getAnotherSelectValues(branches, type_of_data, getColumnName, appendToSelector) {
        if (branches.length) {
            $.ajax({
                ...
                row = '<select data-live-search="true" data-actions-box="true" name="businessUnits[]" class="select2-select form-control kt-bootstrap-select kt_bootstrap_select" required ' + data_type + '  >\n';
