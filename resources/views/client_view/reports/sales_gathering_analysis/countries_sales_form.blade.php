...
<script>
    $(document).on('change', '[name="start_date"],[name="end_date"]', function() {
        ...
        wto = setTimeout(() => {
            var branches = ['all'];
            var type_of_data = "country";
            var getColumnName = 'country';
    ...
    function getAnotherSelectValues(branches, type_of_data, getColumnName, appendToSelector) {
        if (branches.length) {
            $.ajax({
                ...
                row = '<select data-live-search="true" data-actions-box="true" name="countries[]" class="select2-select form-control kt-bootstrap-select kt_bootstrap_select" required ' + data_type + '  >\n';
