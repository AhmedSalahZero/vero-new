    ...
    <div class="kt-portlet__body">
        <div class="tab-content  kt-margin-t-20">
            ...
            <div class="tab-pane @if(!config('app.showTrendCharts')) active @endif" id="kt_apps_contacts_view_tab_2" role="tabpanel">
                <x-table :tableTitle="__('Business Units Sales Trend Analysis Report')" :tableClass="'kt_table_with_no_pagination_no_search'">
                    @slot('table_header')
                    <tr class="table-active">
                        <th>{{ __('Business Units') }}</th>
                    ...
                    <tr>
                        ...
                        @foreach ($total_businessUnits as $date => $total)
                        @php
                                        $chart_data[] = [
	 ...
	 @if($loop->last)
                        <td class="text-center">
                            @@php $totalForUnit[$zone_name] = ($totalForSingleUnit = array_sum($zoone_data['Sales Values']) ?? 0) @end@php
                            {{ number_format($totalForSingleUnit) }}
                        </td>
                        @endif
                        @endforeach
                    </tr>
                    ...
                    @endforeach
                    @php $sumOfTotalsOfUnitSales = 0 @endphp
					
                    <tr>
                        <th class="active-style text-center">{{ __('TOTAL') }}</th>
                        @foreach ($dates as $date )
						@@php
							$currentTotal = $total_businessUnits[$date] ?? 0;
						@end@php
                        <td class="text-center active-style">{{ number_format($currentTotal ?? 0) }}</td>
						
						
												  @php $sumOfTotalsOfUnitSales += ($total_businessUnits[$date] ?? 0) @endphp

                        @if($loop->last)
                        <td class="text-center active-style">
                            {{ number_format($sumOfTotalsOfUnitSales ?? 0) }}
                        </td>
                    ...
                    <tr>
                        ...
                        @foreach ($dates as $date )
                        @php
						$currentTotalGrowthRate = $total_businessUnits_growth_rates[$date] ?? 0;
                                    $chart_data[] = [
                                        'date' => date('d-M-Y', strtotime($date)),
                                        'Total Sales Values' => number_format($total_businessUnits[$date] ?? 0),
                ...
                <x-table :tableTitle="__('Business Units Sales Percentage (%) Against Total Sales')" :tableClass="'kt_table_with_no_pagination_no_search'">
                    @slot('table_header')
                    <tr class="table-active">
                        <th>{{ __('Business Units') }}</th>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
							...
							@if($loop->last)
                        <td class="text-center">
                            {{ $sumOfTotalsOfUnitSales ? number_format((($totalForUnit[$zone_name] / $sumOfTotalsOfUnitSales) * 100  ) , 2) . ' %': 0   }}
                        </td>
                        @endif
                        @endforeach
                    </tr>
                    ...
                    @php
                            ...
                            @endphp
                    <input type="hidden" id="BusinessUnits_Sales_Percentages_data" data-total="{{ json_encode($return) }}">
                ...
                @include('seasonality_table' , ['total_branches'=>$total_businessUnits , 'totalArrys'=>[]])
