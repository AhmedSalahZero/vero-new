...
.dt-bootstrap4>.row>.col-sm-6.text-right{
...
<div class="kt-portlet kt-portlet--tabs">
    ...
    <div class="kt-portlet__body">
        <div class="tab-content  kt-margin-t-20">
                        ...
                        <div class="kt-portlet__body kt-portlet__body--fluid">
                            <div class="kt-widget12">
                                <div class="kt-widget12__chart">
                                    <!-- HTML -->
                                    {{-- <h4>{{ str_replace('_', ' ', $currentName) . ($currentName ==  "percentages" ? ' Against Total Sales' : ' Sales Trend Analysis Chart') }}
                                    </h4> --}}
                                    <div id="{{ $currentName }}_count_chartdiv" class="chartdashboard"></div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="tab-pane @if(!config('app.showTrendCharts')) active @endif" id="trend_tab_id" role="tabpanel">
                <x-table :tableTitle="$reportTitle" :tableClass="'kt_table_with_no_pagination_no_search'">
                    @slot('table_header')
                    <tr class="table-active">
                        <th>{{ $firstColumnNameTitle }}</th>
				...
				@endphp
					...
					<tr>
						...
						@endphp
							...
							@if($loop->last)
							<td class="text-center">
								{{ $totalSales &&  $totalCurrentValue ? number_format($totalCurrentValue / $totalSales * 100  , 2) . ' %' : 0  }}
							</td>
							@endif 
...
<script>
    am4core.ready(function() {
        ...
        var chart = am4core.create("{{ $currentName }}_count_chartdiv", am4charts.XYChart);

        // Increase contrast by taking evey second color
        chart.colors.step = 2;

        // Add data
        chart.data = $('#{{ $currentName }}_data').data('total');
