...
class GeneralAndReservationAssumptionController extends Controller
{
	...
	public function create(Company $company , Request $request,Study $study){
		...
		return view('tradings.general-assumption.form', $this->getViewVars($company,$study));
	}
	public function getOldData(Company $company , Request $request , Study $study)
	{
		...
		return [
			'submitUrl'=>route('trading.store.general.assumption',['company'=>$company->id , 'study'=>$study->id]),
			'dates'=>(object)$dates,
	...
	public function store(Company $company , Request $request,Study $study)
	{
		...
		$redirectRoute = route('trading.create.occupied.properties.with.full.rent.coverage.duration', ['company'=>$study->company->id,'study'=>$study->id]);
