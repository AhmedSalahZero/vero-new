...
use App\Http\Requests\Tradings\StoreExpenseNamesRequest;
use App\Models\Company;
use App\Models\Trading\ExpenseName;
...
class ExpenseController extends Controller
{
    ...
    public function index(Company $company , Request $request){
        ...
        return view('tradings.expense-structure.index', [
			'company'=>$company,
			'items'=>$items,
			'title'=>__('Expenses'),
	...
	public function create(Company $company , Request $request){
		...
		return view('tradings.expense-structure.form', array_merge($this->getViewVars($company),['inEditMode'=>false]));
	}
	protected function getViewVars(Company $company,$expenseType=null,$expenseNames=[]){
		return [
			...
			'title'=>__('Expenses'),
			'storeRoute'=>$expenseType ? route('trading.update.expense.names',['company'=>$company->id,'expenseType'=>$expenseType]) :route('trading.store.expense.names',['company'=>$company->id]),
		];
	}
	public function store(Company $company , StoreExpenseNamesRequest $request)
	{
		...
		return response()->json([
			'redirectTo'=>route('trading.view.expense.names',['company'=>$company->id])
		]);
	}
	...
	public function edit(Request $request , Company $company , string $expenseType ){
		...
		return view('tradings.expense-structure.form', array_merge(['inEditMode'=>true],$this->getViewVars($company,$expenseType,$expenseNames)));
	}
	...
	public function destroy(Request $request,Company  $company ,  $expenseType  ){
		$isExist = DB::connection(TRADING_CONNECTION_NAME)->table('expenses')->where('company_id',$company->id)->where('expense_category',$expenseType)->count();
		if($isExist){
