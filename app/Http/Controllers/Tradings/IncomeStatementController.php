...
use App\Models\Trading\Manpower;
...
class IncomeStatementController extends Controller
{
    public function index(Company $company, Study $study,$onlyViewVars = false )
    {
        ...
        $loanSchedulePayments = DB::connection(TRADING_CONNECTION_NAME)->table('loan_schedule_payments')->selectRaw('portfolio_loan_type,revenue_stream_type,interestAmount,securitization_date_index,endBalance')->where('study_id', $study->id)->get()->toArray();
		...
		// $cashAndBankNonPerformingInterest = $cashAndBankOpeningBalance ? $cashAndBankOpeningBalance->non_performing_interests: [];
	
		// $tableDataFormatted[0]['sub_items']['cash-and-banks-opening-interest']['options']['title'] = __('Existing Portfolio Interest');
		// $tableDataFormatted[0]['sub_items']['cash-and-banks-opening-interest']['data']= HArr::sumAtDates([$cashAndBankInterest,$cashAndBankNonPerformingInterest],$sumKeys);
        ...
        // $directFactoringBreakdown = DB::connection(TRADING_CONNECTION_NAME)->table('direct_factoring_breakdowns')
		...
		// $interestCosts  = DB::connection(TRADING_CONNECTION_NAME)->table('direct_factoring_breakdowns')->where('study_id',$study->id)->pluck('bank_interest_expense')->toArray();
        ...
        // $columnName = 'rent_revenues';
		$formattedResult = [];
		$incomeStatementReport = $study->incomeStatementReport;
		$fullCoverageRentRevenues = $incomeStatementReport? json_decode($incomeStatementReport->full_coverage_rent_revenues,true) : [];
		$fullCoverageRentRevenues = $fullCoverageRentRevenues ? $fullCoverageRentRevenues : [];
		$partialCoverageRentRevenues = $incomeStatementReport? json_decode($incomeStatementReport->partial_coverage_rent_revenues,true) : [];
		$partialCoverageRentRevenues = $partialCoverageRentRevenues ? $partialCoverageRentRevenues : [];
		$toBeDeliveredRentRevenues = $incomeStatementReport? json_decode($incomeStatementReport->to_be_delivered_rent_revenues,true) : [];
		$toBeDeliveredRentRevenues = $toBeDeliveredRentRevenues ? $toBeDeliveredRentRevenues : [];
		$propertyForecastedRentRevenues = $incomeStatementReport? json_decode($incomeStatementReport->property_forecasted_rent_revenues,true) : [];
		$propertyForecastedRentRevenues = $propertyForecastedRentRevenues ? $propertyForecastedRentRevenues : [];
		...
		// foreach($formattedResult as $monthIndex => $val) {
			$tableDataFormatted[0]['sub_items']['Property Contract Full Rent Coverage Contracts']['data'] =$fullCoverageRentRevenues ;
			$tableDataFormatted[0]['sub_items']['Property Contract Full Rent Coverage Contracts']['year_total'] = HArr::sumPerYearIndex($fullCoverageRentRevenues, $yearWithItsMonths);
		// }
		// $formattedResult =[];
		// PropertyContractPartialRentRenewal::getPartialRentCoveragesAmounts($study, $columnName, $formattedResult);
		// foreach($formattedResult as $monthIndex => $val) {
			$tableDataFormatted[0]['sub_items']['Property Contract Partial Rent Coverage Contracts']['data'] = $partialCoverageRentRevenues;
			$tableDataFormatted[0]['sub_items']['Property Contract Partial Rent Coverage Contracts']['year_total'] = HArr::sumPerYearIndex($partialCoverageRentRevenues, $yearWithItsMonths);
		// }
		// $formattedResult = [];
		// PropertyToBeDelivered::getToBeDeliveredCoveragesAmounts($study, $columnName, $formattedResult);
		// foreach($formattedResult as $monthIndex => $val) {
			$tableDataFormatted[0]['sub_items']['Property To Be Delivered Rent Revenues']['data'] = $toBeDeliveredRentRevenues;
			$tableDataFormatted[0]['sub_items']['Property To Be Delivered Rent Revenues']['year_total'] = HArr::sumPerYearIndex($toBeDeliveredRentRevenues, $yearWithItsMonths);
		// }
		// $formattedResult = [];
		// ForecastedProperty::getForecastedPropertiesCoveragesAmounts($study, $columnName, $formattedResult);
		// foreach($formattedResult as $monthIndex => $val) {
			$tableDataFormatted[0]['sub_items']['Forecasted Properties Investments Rent Revenues']['data'] = $propertyForecastedRentRevenues;
			$tableDataFormatted[0]['sub_items']['Forecasted Properties Investments Rent Revenues']['year_total'] = HArr::sumPerYearIndex($propertyForecastedRentRevenues, $yearWithItsMonths);
		// }
		...
		// }
				$totalExpenses = [];
        $expenses = DB::connection(TRADING_CONNECTION_NAME)->table('expenses')->where('study_id',$study->id)->join('expense_names', 'expense_names.id', '=', 'expenses.expense_name_id')->selectRaw('expenses.expense_category,expense_names.name as name,expenses.relation_name,expenses.monthly_repeating_amounts,expenses.total_after_vat,payload')->where('expenses.model_id', $study->id)->where('expenses.model_name', 'Study')->get()->toArray();
        ...
        // $eclExpenses = DB::connection(TRADING_CONNECTION_NAME)->table('ecl_and_new_portfolio_funding_rates')->where('study_id', $study->id)->pluck('monthly_ecl_values')->toArray();
        // $totalEclExpenses = [];
		...
		// $NonPerformingTotalExistingEcl = $incomeStatementReport ? $incomeStatementReport->non_performing_existing_ecl_expenses : [];
		// $totalEclExpenses = HArr::sumAtDates([$totalEclExpenses,$totalExistingEcl,$NonPerformingTotalExistingEcl],$sumKeys);
        ...
        $fixedAssetDepreciations = DB::connection(TRADING_CONNECTION_NAME)->table('fixed_assets')->where('study_id', $study->id)->pluck('total_monthly_depreciations')->toArray();
        ...
        $fixedAssetOpeningBalancesAdminDepreciations = DB::connection(TRADING_CONNECTION_NAME)->table('fixed_asset_opening_balances')->where('study_id', $study->id)->pluck('statement')->toArray();
        ...
        $tableDataFormatted[$eclAndDepreciationOrderIndex]['main_items'][$eclAndDepreciationKey]['options']['title'] = __('Depreciation Cost');
        ...
        */
        ...
        $openingLoans = DB::connection(TRADING_CONNECTION_NAME)->table('long_term_loan_opening_balances')->where('study_id', $study->id)->pluck('interests')->toArray();
        $openingLoansTotal=[];
        ...
        */
		...
		$annuallyCorporateTaxes = $incomeStatementReport ? (array)$incomeStatementReport->corporate_taxes: [];
        ...
        $retainedEarningOpening = DB::connection(TRADING_CONNECTION_NAME)->table('equity_opening_balances')->where('study_id', $study->id)->first();
		...
		$viewVars = [
			'exportRouteName'=>'export.trading.forecast.income.statement',
            'company'=>$company,
            'studyMonthsForViews'=>$studyMonthsForViews,
            ...
            'defaultClasses'=>$defaultClasses,
			'nextButton' => [
				'link'=>route('trading.cash.in.out.flow.result',['company'=>$company->id,'study'=>$study->id]),
				'title'=>__('Go To Cashflow')
			]
        ] ;
        return view('tradings.income-statement.cash-flow',$viewVars );
    }
	public function exportReport(Request $request,Company $company , Study $study )
	{
		...
		$formattedData = $this->index($company,$study,true)['tableDataFormatted'];
		$reportType = 'Income Statement Report';
	...
	public function viewPreviousTwoYearsIncomeStatement(Company $company , Study $study)
	{
		...
		$viewVars = array_merge($viewVars , [
		  ...
		  return view('tradings.income-statement.previous_two_years_income_statements',$viewVars );
	}
