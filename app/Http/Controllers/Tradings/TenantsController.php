...
class TenantsController extends Controller
{
    ...
    public function index(Company $company , Request $request){
        ...
        return view('tradings.tenants.index', [
			'company'=>$company,
			'tenants'=>$tenants,
			'title'=>__('Tenants'),
	...
	public function create(Company $company , Request $request){
		...
		return view('tradings.tenants.form', [
			'company'=>$company,
			'title'=>__('Tenants'),
			'storeRoute'=>route('trading.store.tenants',['company'=>$company->id]),
			'inEditMode'=>false,
			'tenants'=>[]
		]);
	}
	...
	public function store(Company $company , StoreExpenseNamesRequest $request)
	{
		...
		return response()->json([
			'redirectTo'=>route('trading.view.tenants',['company'=>$company->id])
		]);
	}
	...
	public function edit(Request $request , Company $company , Tenant $tenant ){
		return view('tradings.tenants.form', [
			'company'=>$company,
			'title'=>__('Tenants'),
			'storeRoute'=>route('trading.update.tenants',['company'=>$company->id,'tenant'=>$tenant->id]),
			'inEditMode'=>true,
			'tenants'=>[$tenant]
		]);
	}
