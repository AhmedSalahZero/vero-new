...
class ExpensesController extends Controller
{
    ...
    {
        return view('tradings.expenses.form', $this->getViewVars($company, $study));
    }
     public function expensesGetVueOldData(Company $company, Request $request, Study $study)
    {
      ...
      foreach(getTypesForValuesForTrading() as $expenseType => $expenseArr){
        ...
        return response()->json([
           'model'=>$model,
		   'submitUrl'=>route('trading.store.expenses',['company'=>$company->id,'study'=>$study->id]),
 ...
 //          'revenueStreamsPerBusinessUnits'=>$revenueStreamsPerBusinessUnits // for selects
    ...
    public function store(
		...
		return response()->json([
			'redirectTo'=>route('trading.create.ffe.fixed.assets', ['company'=>$company->id,'study'=>$study->id])
		]);
