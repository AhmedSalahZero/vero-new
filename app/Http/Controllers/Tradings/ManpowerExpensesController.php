...
class ManpowerExpensesController extends Controller
{
	public function create(Company $company , Request $request,Study $study){
		return view('tradings.manpower.form', $this->getViewVars($company,$study));
	}
	public function getOldData(Company $company , Request $request , Study $study)
	{
		...
		return [
			'submitUrl'=>route('store.department.positions.for.trading',['company'=>$company->id,'study'=>$study->id]),
			'dates'=>(object)$dates,
	...
	public function storeDepartmentPositions(Company $company , Request $request,Study $study){
		...
		if($request->get('submit_button') == 'save'){
			return response()->json([
				'redirectTo'=>route('view.manpower.for.trading',['company'=>$company->id,'study'=>$study->id])
			]);
		}
		return response()->json([
			'redirectTo'=>route('trading.create.expenses',['company'=>$company->id,'study'=>$study->id])
		]);
