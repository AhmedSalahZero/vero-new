...
use App\Models\Trading\Property;
use App\Models\Trading\PropertyDueInstallment;
...
class DashboardController extends Controller
{
	...
	// 	DB::connection(TRADING_CONNECTION_NAME)->table('property_dashboard')->updateOrInsert([
	...
	// 	]);
    //     return view('tradings.dashboard.property_dashboard', [
    ...
    {
		return view('tradings.dashboard.property_cashflow_dashboard', [
			'company' => $company,
	...
	public function getCashflowForecastDashboardOldData(Company $company, Request $request)
	{
		...
		$propertyDashboard = DB::connection(TRADING_CONNECTION_NAME)->table('property_dashboard')->where('company_id', $company->id)->first();
		...
		return response()->json([
			'submitUrl'=>route('trading.dashboard.cashflow-forecast.submit', ['company'=>$company->id]),
			'dates'=>$dates,
			'model'=>[
	...
	{
		DB::connection(TRADING_CONNECTION_NAME)->table('property_dashboard')->where('company_id', $company->id)->update([
		...
		return response()->json([
			'success'=>true,
			'redirectTo'=>route('trading.view.property.cashflow.forecast.dashboard', ['company'=>$company->id]),
		]);
	}
    ...
    public function view(Request $request, Company $company, Study $study)
    {
        ...
        $withSensitivity = $request->routeIs('trading.view.results.dashboard.with.sensitivity') ;
