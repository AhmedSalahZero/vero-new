...
use App\Http\Requests\Tradings\StoreDepartmentsRequest;
use App\Models\Company;
use App\Models\Trading\Department;
use App\Models\Trading\Position;
...
class DepartmentController extends Controller
{
    ...
    public function index(Company $company , Request $request){
        ...
        return view('tradings.departments.index', [
			'company'=>$company,
	...
	public function create(Company $company , Request $request,string $type){
		...
		return view('tradings.departments.form', $this->getViewVars($company,$type,null));
	}
	protected function getViewVars(Company $company,string $type,$model = null){
		...
		return [
			...
			'type'=>$type,
			'title'=>__('Departments'),
			'storeRoute'=>isset($model) ? route('trading.update.departments',['company'=>$company->id,'department'=>$model->id,'type'=>$type]) :route('trading.store.departments',['company'=>$company->id,'type'=>$type]),
		];
	}
	public function store(Company $company , StoreDepartmentsRequest $request,string $type)
	{
		...
		return response()->json([
			'redirectTo'=>route('trading.view.departments',['company'=>$company->id])
		]);
	}
	...
	public function edit(Request $request , Company $company , Department $department,string $type){
		...
		// 	return view('tradings.microfinance-departments.form', $this->getViewVars($company,$type,$department));
		// }
		return view('tradings.departments.form', $this->getViewVars($company,$type,$department));
	}
	...
	public function destroy(Request $request,Company  $company , Department $department  ){
		...
		$department->positions->each(function(Position $position) use ($company,&$canBeDeleted){
			$isExist = DB::connection(TRADING_CONNECTION_NAME)->table('manpowers')->where('company_id',$company->id)->where('position_id',$position->id)->count();
			if($isExist){
				$canBeDeleted = false ;
			}
