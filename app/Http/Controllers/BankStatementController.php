...
class BankStatementController
{
    ...
    public function updateBankStatementRow(Company $company, Request $request)
    {
		 ...
		 */
		if($bankStatementRecord->interest_journal_entry_id && $company->hasOdooIntegrationCredentials()){
			(new CashExpenseOdooService($company))->unlink($bankStatementRecord->interest_journal_entry_id);
		}elseif($bankStatementRecord->interest_journal_entry_id){
			Log::info('Bank statement row updated without unlinking its Odoo interest entry: no Odoo integration credentials', [
