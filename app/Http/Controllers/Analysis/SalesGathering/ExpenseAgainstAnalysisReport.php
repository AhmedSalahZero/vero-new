...
class ExpenseAgainstAnalysisReport
{
    ...
    public function viewOneAndTwoSelectorAndThreeSelectorAndComparing(Company $company,string $firstColumn , ?string $secondColumn = null , ?string $thirdColumn = null)
	{
            ...
            $type = $firstColumn;
			$firstColumnViewName = $this->formatTitle($firstColumn) ;
			$secondColumnViewName = $this->formatTitle($secondColumn)  ;
			$thirdColumnViewName = $this->formatTitle($thirdColumn)  ;
	...
	public function oneSelectorResult(Request $request, Company $company , $array=false )
    {
		...
		$firstColumnNameTitle = $this->formatTitle($firstColumnName) ;
		...
		$intervalName = HVero::capitializeType($intervalName);
	...
	{
		$viewName = $this->formatTitle($columnName) . ' Breakdown Analysis';
...
{
    return trim(ucwords(HVero::capitializeType(str_replace('_', ' ', $string))));
}
