...
class DepartmentController extends Controller
{
	...
	protected function getViewVars(Company $company,string $type,$model = null){
		...
		return [
			...
			'type'=>$type,
			'title'=>__('Departments'),
			'storeRoute'=>isset($model) ? route('non.banking.update.departments',['company'=>$company->id,'department'=>$model->id,'type'=>$type]) :route('non.banking.store.departments',['company'=>$company->id,'type'=>$type]),
		];
	}
	public function store(Company $company , StoreDepartmentsRequest $request,string $type)
	{
		...
		return response()->json([
			'redirectTo'=>route('non.banking.view.departments',['company'=>$company->id])
		]);
	}
