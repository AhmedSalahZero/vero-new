...
class ExpenseController extends Controller
{
	...
	protected function getViewVars(Company $company,$expenseType=null,$expenseNames=[]){
		return [
			...
			'title'=>__('Expenses'),
			'storeRoute'=>$expenseType ? route('non.banking.update.expense.names',['company'=>$company->id,'expenseType'=>$expenseType]) :route('non.banking.store.expense.names',['company'=>$company->id]),
		];
	}
	public function store(Company $company , StoreExpenseNamesRequest $request)
	{
		...
		return response()->json([
			'redirectTo'=>route('non.banking.view.expense.names',['company'=>$company->id])
		]);
	}
