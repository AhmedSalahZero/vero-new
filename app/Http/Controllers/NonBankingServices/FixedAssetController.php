...
class FixedAssetController extends Controller
{
	...
	protected function getViewVars(Company $company,?FixedAssetName $fixedAssetName = null){
		return [
			...
			'storeRoute'=>$fixedAssetName ? route('non.banking.update.fixed.asset.names',['company'=>$company->id,'fixedAssetName'=>$fixedAssetName->id]) : route('non.banking.store.fixed.asset.names',['company'=>$company->id]),
		];
	}
	public function store(Company $company , StoreFixedAssetNamesRequest $request)
	{
		...
		return response()->json([
			'redirectTo'=>route('non.banking.view.fixed.asset.names',['company'=>$company->id])
		]);
	}
