...
use App\Models\PropertyManagement\Consolidation;
...
class StudyController extends Controller
{
	...
	//	$studies =  $studies->filterByDateColumn('study_start_date',$startDate,$endDate) ;
        ...
        return view('property_managements.study.index', [
			'company'=>$company,
	...
	//		'formattedExistingBranches'=>$formattedExistingBranches,
			...
			'model'=>$model,
			'storeRoute'=>route('store.property.management.services',['company'=>$company->id]),
			'navigators' => [],
	...
	public function create(Company $company , Request $request){
		...
		return view('property_managements.study.form', $this->getViewVars($company,null,$isBusinessPlan));
	}
	public function store(Company $company , StoreStudyRequest $request , ?Study $study = null)
	{
		...
		$incomeStatementExist=DB::connection('property_management')->table('income_statement_reports')->where('study_id',$model->id)->first();
		if(!$incomeStatementExist){
			DB::connection('property_management')->table('income_statement_reports')->insert([
				'study_id'=>$model->id,
				'company_id'=>$model->company->id,
			]);
			
			DB::connection('property_management')->table('cashflow_statement_reports')->insert([
				'study_id'=>$model->id,
				'company_id'=>$model->company->id,
			]);
		...
		return response()->json([
			'redirectTo'=>route('property.management.create.general.assumption',['company'=>$company->id,'study'=>$model->id])
		]);
	}
	public function edit(Company $company , Request $request,Study $study){
		return view('property_managements.study.form', $this->getViewVars($company,$study));
	}
	...
	public function destroy(Request $request , Company $company,Study $study)
	{
		...
		$study->delete();
		return redirect()->route('property.management.view.study',['company'=>$company->id,'active'=>$active]);
