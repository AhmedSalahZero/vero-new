...
use App\Models\PropertyManagement\Manpower;
...
class IncomeStatementController extends Controller
{
    public function index(Company $company, Study $study,$onlyViewVars = false )
    {
        ...
        $loanSchedulePayments = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('loan_schedule_payments')->selectRaw('portfolio_loan_type,revenue_stream_type,interestAmount,securitization_date_index,endBalance')->where('study_id', $study->id)->get()->toArray();
        ...
        // $directFactoringBreakdown = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('direct_factoring_breakdowns')
		...
		// $interestCosts  = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('direct_factoring_breakdowns')->where('study_id',$study->id)->pluck('bank_interest_expense')->toArray();
        ...
        // $eclExpenses = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('ecl_and_new_portfolio_funding_rates')->where('study_id', $study->id)->pluck('monthly_ecl_values')->toArray();
        // $totalEclExpenses = [];
        ...
        $fixedAssetDepreciations = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('fixed_assets')->where('study_id', $study->id)->pluck('total_monthly_depreciations')->toArray();
        ...
        $fixedAssetOpeningBalancesAdminDepreciations = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('fixed_asset_opening_balances')->where('study_id', $study->id)->pluck('statement')->toArray();
        ...
        */
        ...
        $openingLoans = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('long_term_loan_opening_balances')->where('study_id', $study->id)->pluck('interests')->toArray();
        $openingLoansTotal=[];
        ...
        */
        ...
        $retainedEarningOpening = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('equity_opening_balances')->where('study_id', $study->id)->first();
		...
		$viewVars = [
			'exportRouteName'=>'export.property.management.forecast.income.statement',
            'company'=>$company,
            'studyMonthsForViews'=>$studyMonthsForViews,
       ...
       //     'createRoute'=>route('create.financial.planning.study', ['company'=>$company->id]),
            ...
            'defaultClasses'=>$defaultClasses,
			'nextButton' => [
				'link'=>route('property.management.cash.in.out.flow.result',['company'=>$company->id,'study'=>$study->id]),
				'title'=>__('Go To Cashflow')
			]
        ] ;
        return view('property_managements.income-statement.cash-flow',$viewVars );
    }
