...
use App\Models\PropertyManagement\LongTermLoanOpeningBalance;
use App\Models\PropertyManagement\OtherCreditsOpeningBalance;
use App\Models\PropertyManagement\OtherDebtorsOpeningBalance;
use App\Models\PropertyManagement\OtherLongTermAssetsOpeningBalance;
use App\Models\PropertyManagement\OtherLongTermLiabilitiesOpeningBalance;
...
class OpeningBalancesController extends Controller
{
    ...
    {
        return view('property_managements.openingBalances.form', array_merge($study->getOpeningBalancesViewVars(), ['inEditMode'=>false]));
    }
    ...
    public function store(Company $company, StoreOpeningBalancesRequest $request, Study $study)
    {
        ...
        $totalExistingPortfolioCollection = HArr::sumAtDates([$totalExistingPortfolioPrinciple ], $sumKeys);
        ...
        $totalExistingPortfolioLoansPayments =$totalExistingPortfolioLoansPrinciple;
        
        
        
        
        DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('cashflow_statement_reports')->where('study_id', $study->id)->update([
        ...
        DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('income_statement_reports')->where('study_id', $study->id)->update([
        ...
        return redirect()->route('view.property.management.forecast.income.statement', ['company'=>$company->id,'study'=>$study->id]);
	...
	protected function checkSettlementsOrPrincipleValidation(Request $request):array 
	{
		...
		foreach($request->get('supplierPayableOpeningBalances',[]) as $index=>$supplierPayableOpeningBalances){
			...
			if($settlements>$amount){
				return [
					'status'=>false ,
					'message' => __('Principle Must be Less Than Or Equal Portfolio Loans Outstanding In ODAs & Suppliers Outstanding [Portfolio Loans Outstanding =  ' . number_format($amount) . ' And Total Principle = '  . number_format($settlements))
				];
			}
		}
		foreach($request->get('otherLongTermAssetsOpeningBalances',[]) as $index=>$otherLongTermAssetsOpeningBalances){
