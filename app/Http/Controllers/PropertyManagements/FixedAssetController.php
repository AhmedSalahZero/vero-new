...
use App\Http\Requests\PropertyManagements\StoreFixedAssetNamesRequest;
...
class FixedAssetController extends Controller
{
	...
	protected function applyFilter(Request $request,Collection $collection):Collection{
	  ...
	  public function index(Company $company , Request $request){
        ...
        return view('property_managements.fixed-asset-structure.index', [
			'company'=>$company,
	...
	public function create(Company $company , Request $request){
		...
		return view('property_managements.fixed-asset-structure.form', array_merge($this->getViewVars($company),['inEditMode'=>false]));
	}
	protected function getViewVars(Company $company,?FixedAssetName $fixedAssetName = null){
		return [
			...
			'storeRoute'=>$fixedAssetName ? route('property.management.update.fixed.asset.names',['company'=>$company->id,'fixedAssetName'=>$fixedAssetName->id]) : route('property.management.store.fixed.asset.names',['company'=>$company->id]),
		];
	}
	...
	//		$isBranchAsset = $fixedAsset['is_branch_asset'][0]??0 ;
			DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('fixed_asset_names')->insert([
		...
		return response()->json([
			'redirectTo'=>route('property.management.view.fixed.asset.names',['company'=>$company->id])
		]);
	}
	...
	public function edit(StoreFixedAssetNamesRequest $request , Company $company , FixedAssetName $fixedAssetName ){
		...
		return view('property_managements.fixed-asset-structure.form', array_merge($this->getViewVars($company,$fixedAssetName),['inEditMode'=>true,'fixedAssetNames'=>$fixedAssetNames]));
	}
	public function update(Request $request , Company $company , FixedAssetName $fixedAssetName){
		...
		DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('fixed_asset_names')->where('id',$fixedAssetName->id)->update([
			'name'=>$name ,
	...
	public function destroy(Request $request,Company  $company ,  FixedAssetName $fixedAssetName  ){
		$isExist = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('fixed_assets')->where('company_id',$company->id)->where('name_id',$fixedAssetName->id)->count();
		if($isExist){
