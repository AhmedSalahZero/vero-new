...
class CopyStudyController extends Controller
{
    public function index(Request $request,Company $company, Study $study)
    {
       $id = $study->id ;
		...
		$tablesWithOnlyHospitalitySectorAsForeignKey =getTableNamesThatHasColumn('study_id',PROPERTY_MANAGEMENT_CONNECTION_NAME) ;
		
		$newProject->save();
		foreach( $tablesWithOnlyHospitalitySectorAsForeignKey as $tableName){
			
			$allData = [];
			$rows = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table($tableName)->where('study_id', $id)->get(); // استرجاع الصف ككائن stdClass
			foreach($rows as $row){
			...
			DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table($tableName)->insert($allData); // إدراج نسخة جديدة
