...
class ExpenseController extends Controller
{
    ...
    public function index(Company $company , Request $request){
        ...
        return view('property_managements.expense-structure.index', [
			'company'=>$company,
			'items'=>$items,
			'title'=>__('Expenses'),
	...
	public function create(Company $company , Request $request){
		...
		return view('property_managements.expense-structure.form', array_merge($this->getViewVars($company),['inEditMode'=>false]));
	}
	protected function getViewVars(Company $company,$expenseType=null,$expenseNames=[]){
		return [
			...
			'title'=>__('Expenses'),
			'storeRoute'=>$expenseType ? route('property.management.update.expense.names',['company'=>$company->id,'expenseType'=>$expenseType]) :route('property.management.store.expense.names',['company'=>$company->id]),
		];
	}
	public function store(Company $company , StoreExpenseNamesRequest $request)
	{
		...
		return response()->json([
			'redirectTo'=>route('property.management.view.expense.names',['company'=>$company->id])
		]);
	}
	...
	public function edit(Request $request , Company $company , string $expenseType ){
		...
		return view('property_managements.expense-structure.form', array_merge(['inEditMode'=>true],$this->getViewVars($company,$expenseType,$expenseNames)));
	}
	...
	public function destroy(Request $request,Company  $company ,  $expenseType  ){
		$isExist = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('expenses')->where('company_id',$company->id)->where('expense_category',$expenseType)->count();
		if($isExist){
