...
begin 
 ...
 delimiter ; 
drop trigger if exists refresh_calculation_before_delete_loan_statements ;
  delimiter //  
  
create  trigger refresh_calculation_before_delete_loan_statements before delete on `loan_statements` for each row 
begin 
	delete from `temp_deleted_statements` where company_id = old.company_id and table_name = 'loan_statements';
	insert into `temp_deleted_statements` (company_id,table_name,deleted_id) values (old.company_id,'loan_statements',old.id);
end //
 delimiter ; 
