...
begin 
	...
	declare _min_interest_rate decimal(5,2) default 0 ; 
		...
		select date,end_balance into _previous_date, _last_end_balance  from current_account_bank_statements where is_active = 1 and company_id = new.company_id and financial_institution_account_id = new.financial_institution_account_id  and  (full_date < new.full_date or (full_date = new.full_date and id < new.id))   order by full_date desc , id desc  limit 1 ;
		select  count(*) into _count_all_rows from current_account_bank_statements where is_active = 1 and company_id = new.company_id  and financial_institution_account_id = new.financial_institution_account_id  and  (full_date < new.full_date or (full_date = new.full_date and id < new.id))   order by full_date desc , id desc limit 1 ;
