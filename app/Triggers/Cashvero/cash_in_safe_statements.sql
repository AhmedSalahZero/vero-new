...
begin 
	...
	declare _previous_date date default null ;
		...
		select date,end_balance into _previous_date, _last_end_balance  from cash_in_safe_statements where company_id = new.company_id and currency = new.currency and branch_id = new.branch_id  and  (full_date < new.full_date or (full_date = new.full_date and id < new.id))   order by full_date desc , id desc limit 1 ;
		...
		select  count(*) into _count_all_rows from cash_in_safe_statements where company_id = new.company_id and currency = new.currency and branch_id = new.branch_id and (full_date < new.full_date or (full_date = new.full_date and id < new.id))   order by full_date desc , id desc limit 1 ;
...
begin 
	...
	-- خده كوبي وبيست وحطة هنا
		...
		declare _previous_date date default null ;
		declare _count_all_rows integer default 0 ; 
		select date,end_balance into _previous_date, _last_end_balance from cash_in_safe_statements where company_id = new.company_id and branch_id = new.branch_id and currency = new.currency and  (full_date < new.full_date or (full_date = new.full_date and id < new.id))  order by full_date desc , id desc limit 1 ;
