...
class PropertyTaxesPayableEndBalance  
{
	...
	public function getPropertyTaxesPayableEndBalance(array $purchase , array $collection , array $dateIndexWithDate,array $dateWithDateIndex , HospitalitySector $hospitalitySector )
	{
		...
		foreach(getIntervalFormatted() as $intervalName=>$intervalNameFormatted){
			...
			foreach ($purchasesForIntervals[$intervalName]  as $date=>$purchaseAtDate) {
				...
				$result[$intervalName]['property_taxes'][$date] = $purchaseAtDate ?? 0 ; 
