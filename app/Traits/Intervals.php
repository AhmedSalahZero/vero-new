...
trait Intervals
{
	...
	protected static function filterIntervalBasedOnStartDateMonth(array $intervalMonths,string $startDateMonth):array {
	...
	public static function intervalsDatesWithLimitMonths($years_intervals_months, $years,$requested_interval,string $limitDates,?string $startDateMonth = null)
    {
		...
		$last_loop_intervals = [
			'monthly'=>self::getMonthsLessThanOrEqual($limitMonth,$intervals['monthly']),
			'quarterly'=>self::getMonthsForQuarterly($limitMonth,$intervals['quarterly']),
			'semi-annually'=>self::getMonthsForSemiAnnually($limitMonth,$intervals['semi-annually']),
			'annually'=>self::getMonthsForAnnually($limitMonth),
		];
...
protected static function getMonthsLessThanOrEqual($limitMonth, $months)
{
    $result = [];
...
protected static function getMonthsForQuarterly($limitMonth, $quarters)
{
    if ($limitMonth <=3) {
...
protected static function getMonthsForSemiAnnually($limitMonth, $quarters)
{
    if ($limitMonth <=6) {
        return ['01-'.$limitMonth];
    }
...
protected static function getMonthsForAnnually($limitMonth)
{
    return ['01-'.$limitMonth];
}
