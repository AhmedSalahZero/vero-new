    ...
    WithTitle
{
    ...
    public function collection()
    {
        ...
        // }) ;
        /** @var \Illuminate\Support\Collection<int, IncomeStatementItem> $mainItemsWithSubItems */

        $mainItemsWithSubItems = $this->incomeStatement->mainItems->where('has_sub_items', 1)->filter(function (IncomeStatementItem $mainItem): IncomeStatementItem {
            $subItemsNames = $mainItem->subItems()
                ->wherePivot('sub_item_type', 'actual')
                ->wherePivot('financial_statement_able_id', $this->incomeStatement->id)
                ->wherePivot('financial_statement_able_item_id', $mainItem->id)
                ->get()
                ->sortBy('pivot.id')
                ->pluck('pivot.sub_item_name', 'pivot.id')
                ->toArray();
        
            $mainItem->setRelation('sub_items_name', $subItemsNames);
        ...
        foreach ($mainItemsWithSubItems as $mainItem) {
			 ...
			 * * For  LARASTAN
			 */
			$subItemNames = isset($mainItem->sub_items_name) ?  $mainItem->sub_items_name : [];
            foreach ($subItemNames as $subItemName) {
