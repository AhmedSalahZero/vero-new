...

{
	...
	public function headings(): array
	{
		...
		$header = [
			...
			//  ,
			 [
				app(Company::class)->labeling_report_title
			 ]
			//  ,[
			// 	getExportDateTime()
			//  ],[
...
public function drawings()
{
	...
	foreach($this->exportData as $index=>$row){
		...
		\Storage::disk('public')->put(
			$imageName,
			\QrCode::format('png')->size(150)->generate($row->getCode($index+1))
		);
	...
	foreach(['labeling_logo_3','labeling_logo_2','labeling_logo_1'] as $logo){
		$currentLog = app(Company::class)->{$logo};
	
		$logoPath = public_path('storage/'.$currentLog) ;
		if(is_null($currentLog) || !file_exists($logoPath)){
			continue;
		}
