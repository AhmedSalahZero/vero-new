...
use App\Models\NonBankingService\RightOfUseAssetOpeningBalance;
...
class Study extends Model
{
        ...
        static::updated(function (self $study) {
            ...
            if ($study->isDirty('company_nature') && $study->isNewCompany()) {
                foreach ([
                    ...
                    'long_term_investments_opening_balances',
    ...
    public function storeFixedLoans(Request $request, string $revenueStreamType, string $relationName, bool $isSensitivity = false, ?array $pricingPerMonths = null):void
    {
        ...
        /**
         * @var EclAndNewPortfolioFundingRate|null $eclAndNewPortfolioFundingRate
         * @var GeneralAndReserveAssumption|null $generalAndReserveAssumption
         */
        ...
        // $bankLendingMarginRates=$generalAndReserveAssumption->getBankLendingMarginRates($this->getBankLendingRateTypeForRevenueStream($revenueStreamType));
    
        // $baseRatesMapping2 = HArr::isAllValuesEqual($baseRatesMapping, $bankLendingMarginRates);
        $baseRatesMapping = $this->generalAndReserveAssumption->getBaseRatesPerMonths($this->getBankLendingRateTypeForRevenueStream($revenueStreamType));
             ...
             //   $yearOrMonthIndex = $isMonthlyStudy ? $monthIndex : $yearIndex;
                ...
                foreach ($revenueIdWitLoanAmounts as $leasingRevenueStreamBreakdownId => $yearIndexWithAmount) {
                    ...
                    if ($eclAndNewPortfolioFundingRate ) {
                        ...
                        $currentMarginRate = $generalAndReserveAssumption->getBankLendingMarginRatesAtYearOrMonthIndex($monthIndex, $bankLendingRateType);
    ...
    public function recalculatePortfolioMortgage($request,$isSensitivity=false)
    {
        ...
        $revenueStreamType = Study::PORTFOLIO_MORTGAGE;
                DB::connection('non_banking_service')->table($loanSchedulePaymentTableName)->where('revenue_stream_type', $revenueStreamType)->where('study_id', $this->id)->delete();
        ...
        $bankMarginRatesPerYears =$this->generalAndReserveAssumption  ?  $this->generalAndReserveAssumption->getBankLendingMarginRates($this->getBankLendingRateTypeForRevenueStream($revenueStreamType)) : [];
    ...
    public function calculateCorporateTaxesFromIncomeStatementReport(IncomeStatementReport $incomeStatementReport, array $yearWithItsMonths, int $startDateAsIndex, int $endDateAsIndex, array $dateIndexWithDate)
    {
        ...
        $expenses = [
			...
			'right_of_user_amortization',
			'rent_interest',
			'new_branches_rent_interest',
			'new_branches_rent_amortization',
        ];
    ...
    public function runIncomeStatementIfFromCashflow()
    {
        if (Request()->has('redirect-to-cashflow')) {
            ...
            return route('non.banking.cash.in.out.flow.result', ['company'=>$this->company->id,'study'=>$this->id]);
        }
    }
    ...
    public function storeVariableLoans(Request $request, string $revenueStreamType, string $relationName, bool $isSensitivity = false):void
    {
		 ...
		 * @var array $dateIndexWithDate
		 */
        $baseRatesMapping = $generalAndReserveAssumption->getBaseRatesPerMonths(GeneralAndReserveAssumption::FACTORING_BANK_LENDING_MARGIN_RATE);
        DB::connection('non_banking_service')->table($loanSchedulePaymentTableName)->where('revenue_stream_type', $revenueStreamType)->where('study_id', $studyId)->delete();
        $operationDates = range($study->getOperationStartDateAsIndex(), $study->getStudyEndDateAsIndex());
    //    $CurrentIndex = -1 ;
        foreach ($operationDurationPerYear as $yearIndex => $yearMonthIndexes) {
     ...
     */
    public function calculateExistingPortfolioEclFor(array $monthlyEclRates, array $totalPortfolioEndBalance, int $openingBalance = 0)
	{
		$monthlyEclValues = [];
		...
		return [
			'result'=>$monthlyEclValues,
			'accumulated_result'=>$accumulatedEclValues,
		];
	}
    public function calculateExistingPortfolioEcl(array $monthlyEclRates, array $totalPortfolioEndBalance, int $openingBalance = 0)
    {
        $result = $this->calculateExistingPortfolioEclFor($monthlyEclRates,$totalPortfolioEndBalance,$openingBalance);
        return [
            'ecl_existing_expenses'=>$result['result'],
            'accumulated_ecl_existing_expenses'=>$result['accumulated_result'],
        ];
        
    }
	 public function calculateNonPerformingExistingPortfolioEcl(array $monthlyEclRates, array $totalPortfolioEndBalance, int $openingBalance = 0)
    {
        $result = $this->calculateExistingPortfolioEclFor($monthlyEclRates,$totalPortfolioEndBalance,$openingBalance);
        return [
            'non_performing_ecl_existing_expenses'=>$result['result'],
            'non_performing_accumulated_ecl_existing_expenses'=>$result['accumulated_result'],
        ];
    ...
    */
    ...
    //     $studyStartDateMonth = (int)(explode('-', $studyStartDateMonth)[1]);
    ...
    public function refreshDirectFactoringLoans($request)
    {
         ...
         */
        $baseRates = $generalAndReserveAssumption->getCbeLendingCorridorRates() ;
        $bankMarginRates = $generalAndReserveAssumption->getBankLendingMarginRates($this->getBankLendingRateTypeForRevenueStream(self::DIRECT_FACTORING));
        ;
        $datesIndexWithYearIndex = $this->getDatesIndexWithYearIndex() ;
        // $dateIndexWithDates = app()->make('dateIndexWithDate');
        $dateIndexWithDates = $this->getDateIndexWithDate() ;
        ...
        foreach ($this->refresh()->directFactoringBreakdowns as $directFactoringBreakdown) {
            ...
            foreach ($directFactoringAmounts as $monthIndex => $currentDirectAmount) {
                ...
                $offsetMonth  =(int)(ceil($category/30)) ;
                $directFactoringStatements[$directFactoringBreakdownId]['direct_factoring_settlements'][$monthIndex + $offsetMonth  ] = $currentDirectAmount;
                ...
                if ($eclAndNewPortfolioFundingRate) {
					...
					$offsetMonth  =(int)(ceil($category/30)) ;
                    $directFactoringBankLoanStatements[$directFactoringBreakdownId]['loan_settlements'][$monthIndex + $offsetMonth  ] = $currentBankLoanAmount;
...
// + $interestExpense
    ...
    //         'title'=>__('Expenses'),
    //         'storeRoute'=>route('non.banking.store.expenses', ['company'=>$company->id , 'study'=>$this->id]),
    //         'yearsWithItsMonths' => $this->getOperationDurationPerYearFromIndexes(),
	...
	public function longTermInvestmentsOpeningBalances():HasMany
    {
        return $this->hasMany(LongTermInvestmentsOpeningBalance::class, 'study_id', 'id');
    }
    ...
    public function rightOfUseAssetOpeningBalances():HasMany
    {
        return $this->hasMany(RightOfUseAssetOpeningBalance::class, 'study_id', 'id');
    }
	public function leaseRentLiabilityOpeningBalances():HasMany
    {
        return $this->hasMany(LeaseRentLiabilityOpeningBalance::class, 'study_id', 'id');
    }
    ...
    public function getOpeningBalancesViewVars():array
    {
        ...
        $rightOfUseAssetOpeningBalances = $this->rightOfUseAssetOpeningBalances;
        $leaseRentLiabilityOpeningBalances = $this->leaseRentLiabilityOpeningBalances;
        ...
        $longTermInvestmentsOpeningBalances = $this->longTermInvestmentsOpeningBalances;
        $equityOpeningBalances = $this->equityOpeningBalances;
        $longTermLoanOpeningBalances = $this->longTermLoanOpeningBalances;
        // $products = $this->products;
        return ['title'=>__('Opening Balances'),'study'=>$this,'vatAndCreditWithholdTaxesOpeningBalances'=>$vatAndCreditWithholdTaxesOpeningBalances
		,'longTermInvestmentsOpeningBalances'=>$longTermInvestmentsOpeningBalances
		,'longTermLoanOpeningBalances'=>$longTermLoanOpeningBalances,'equityOpeningBalances'=>$equityOpeningBalances,'otherLongTermLiabilitiesOpeningBalances'=>$otherLongTermLiabilitiesOpeningBalances,'otherLongTermAssetsOpeningBalances'=>$otherLongTermAssetsOpeningBalances,'otherCreditorsOpeningBalances'=>$otherCreditorsOpeningBalances,'supplierPayableOpeningBalances'=>$supplierPayableOpeningBalances,'rightOfUseAssetOpeningBalances'=>$rightOfUseAssetOpeningBalances,'leaseRentLiabilityOpeningBalances'=>$leaseRentLiabilityOpeningBalances,'otherDebtorsOpeningBalances'=>$otherDebtorsOpeningBalances,'fixedAssetOpeningBalances'=>$fixedAssetOpeningBalances,'cashAndBankOpeningBalances'=>$cashAndBankOpeningBalances];
	...
	public function getCashAndBank():?CashAndBankOpeningBalance
	{
		return $this->cashAndBankOpeningBalances->first();
	}
    public function getCashAndBanksAmount():float
    {
        $cashAndBank   = $this->getCashAndBank();
        return $cashAndBank ? $cashAndBank->cash_and_bank_amount : 0 ;
    }
    public function getCustomerReceivableAmount():float
    {
          $cashAndBank   = $this->getCashAndBank();
        return $cashAndBank ? $cashAndBank->customer_receivable_amount : 0 ;
    }
    public function getCashInOutFlowViewVars()
    {
        ...
        */
        ...
        $cashAndBank = DB::connection(NON_BANKING_SERVICE_CONNECTION_NAME)->table('cash_and_bank_opening_balances')->where('study_id', $this->id)->first(['interests','payload','non_performing_payload','non_performing_interests']);
        if ($cashAndBank) {
            $settlements = json_decode($cashAndBank->payload, true);
            $interests = json_decode($cashAndBank->interests, true);
			 $nonPerformingSettlements = json_decode($cashAndBank->non_performing_payload?:'{}', true);
            $nonPerformingInterests = json_decode($cashAndBank->non_performing_interests?:'{}', true);
            $subItemTotal = HArr::sumAtDates([$settlements,$interests,$nonPerformingSettlements,$nonPerformingInterests], $sumKeys);
		...
		$existingPortfolioCorridorChangesInterest = $incomeStatementReport->interest_corridor_changes?:[];
		$tableDataFormatted[0]['sub_items']['existing-portfolio-corridor-changes-interest']['options']['title'] = __('Existing Portfolio Corridor Changes (Gain / (Loss) )');
		$tableDataFormatted[0]['sub_items']['existing-portfolio-corridor-changes-interest']['data']= $existingPortfolioCorridorChangesInterest;
		$tableDataFormatted[0]['sub_items']['existing-portfolio-corridor-changes-interest']['year_total']= HArr::sumPerYearIndex($existingPortfolioCorridorChangesInterest, $yearWithItsMonths);
        ...
        // $longTermInvestmentsOpeningBalances = DB::connection(NON_BANKING_SERVICE_CONNECTION_NAME)->table('long_term_investments_opening_balances')->where('study_id', $this->id)->pluck('statement', 'name')->toArray();
        // $totalLongTermInvestmentsOpeningBalances = HArr::formatMultiSubItemsPerKey($longTermInvestmentsOpeningBalances, $sumKeys, ['monthly','payment']);
        // foreach ($totalLongTermInvestmentsOpeningBalances as $keyName => $subItemTotal) {
        //     $id = 'long_term_investments_opening_balances'.$keyName;
        //     $tableDataFormatted[0]['sub_items'][$id]['data'] = $subItemTotal;
        //     $tableDataFormatted[0]['sub_items'][$id]['options']['title'] = $keyName;
        //     $tableDataFormatted[0]['sub_items'][$id]['year_total'] = HArr::sumPerYearIndex($subItemTotal, $yearWithItsMonths);
        // }
		...
		$tableDataFormatted[1]['sub_items'][$existingPortfolioCorridorChangesInterestTitle]['year_total']= HArr::sumPerYearIndex($loansExistingPortfolioCorridorChangesInterest, $yearWithItsMonths);
		...
		$rightOfUserRentPayment= LeaseRentLiabilityOpeningBalance::where('study_id',$this->id)->first();
		$rightOfUserRentPayment = $rightOfUserRentPayment ? $rightOfUserRentPayment->rent_payment : [];
		  $tableDataFormatted[1]['sub_items'][__('Existing Branches Right Of Use Rent Payment')]['data'] = $rightOfUserRentPayment;
		  $tableDataFormatted[1]['sub_items'][__('Existing Branches Right Of Use Rent Payment')]['year_total'] = HArr::sumPerYearIndex($rightOfUserRentPayment, $yearWithItsMonths);
	  
	
		  $newBranchesRightOfUserRentPayment = $this->cashflowStatementReport ? (array)$this->cashflowStatementReport->new_branches_rent_payments : [];
		  $tableDataFormatted[1]['sub_items'][__('New Branches Right Of Use Rent Payment')]['data'] = $newBranchesRightOfUserRentPayment;
		  $tableDataFormatted[1]['sub_items'][__('New Branches Right Of Use Rent Payment')]['year_total'] = HArr::sumPerYearIndex($newBranchesRightOfUserRentPayment, $yearWithItsMonths);
        ...
        */
        ...
        $tableDataFormatted[$currentTabIndex]['main_items'][$currentTabId]['options'] = array_merge([
           'title'=>__('Calculated Extra ODAs Withdrawal')
        ], $defaultNumericInputClasses);
   ...
   //         'monthly_working_capital_injection'=>$workingCapitalInjection??[],
        ...
        return  [
          ...
          'exportRouteName'=>'export.non.banking.forecast.cash.in.out',
          'nextButton' => [
                'link'=>route('non.banking.balance.sheet.result', ['company'=>$this->company->id,'study'=>$this->id]),
    ...
    public function getBalanceSheetViewVars($onlyViewVars = false)
    {
        ...
        foreach ($fixedAssetNames as $fixedAssetName) {
            ...
            foreach ($currentFixedAssets as $currentFixedAsset) {
				$currentEndBalance = json_decode($currentFixedAsset->depreciation_statement, true);
				$currentEndBalance = $currentEndBalance['end_balance'] ?? [];
        ...
        */
         ...
         */
        $totalLongTermInvestmentsOpeningBalances = DB::connection(NON_BANKING_SERVICE_CONNECTION_NAME)->table('long_term_investments_opening_balances')->where('study_id', $this->id)->pluck('statement')->toArray();
        $totalLongTermInvestmentsOpeningBalances= HArr::formatMultiSubItems($totalLongTermInvestmentsOpeningBalances, $sumKeys, ['monthly','end_balance']);
        $tableDataFormatted[$currentTabIndex]['main_items'][$currentTabIndex]['options']['title'] = __('Long Term Investments');  // statement from other long term assets [statement]
        $tableDataFormatted[$currentTabIndex]['main_items'][$currentTabIndex]['data'] = $totalLongTermInvestmentsOpeningBalances;  // statement from other long term assets [statement]
        $tableDataFormatted[$currentTabIndex]['main_items'][$currentTabIndex]['year_total'] =$totalLongTermInvestmentsOpeningBalancesPerYears= HArr::getPerYearIndexForEndBalance($totalLongTermInvestmentsOpeningBalances, $yearWithItsMonths);  // statement from other long term assets [statement]
        $currentTabIndex++;
        /**
          * * End Long Term Investments
          */
        ...
        $tableDataFormatted[$nonCurrentAssetTabIndex]['main_items'][$nonCurrentAssetTabIndex]['data'] =  $totalNonCurrentAssets = HArr::sumAtDates([$totalFixedAssets ,$totalOtherLongTermAssets,$totalLongTermInvestmentsOpeningBalances], $sumKeys) ;
		...
		$NonPerformingCashAndBankOpeningBalances = DB::connection(NON_BANKING_SERVICE_CONNECTION_NAME)->table('cash_and_bank_opening_balances')->where('study_id', $this->id)->pluck('non_performing_statement')->toArray();
        ...
        */
		...
		$rightOfUserStatement = RightOfUseAssetOpeningBalance::where('study_id',$this->id)->first();
		$rentRightOfUse = $rightOfUserStatement  ? $rightOfUserStatement->statement['monthly']['end_balance'] : [];
		$tableDataFormatted[$currentTabIndex]['sub_items']['Existing Branches Rent Right Of Use']['data']  = $rentRightOfUse ;
		$tableDataFormatted[$currentTabIndex]['sub_items']['Existing Branches Rent Right Of Use']['year_total']  = HArr::getPerYearIndexForEndBalance($rentRightOfUse, $yearWithItsMonths) ;
		$totalNewBranchesRentRightOfUserEndBalance = [];
		$totalRentLiabilityEndBalance = [];
		$this->newBranchMicrofinanceOpeningProjections->each(function($newBranchMicrofinanceOpeningProjection) use ($sumKeys,&$totalRentLiabilityEndBalance,&$totalNewBranchesRentRightOfUserEndBalance) {
			$totalNewBranchesRentRightOfUserEndBalance = HArr::sumAtDates([$totalNewBranchesRentRightOfUserEndBalance,$newBranchMicrofinanceOpeningProjection->right_of_use_statement['end_balance']??[]], $sumKeys);
			$totalRentLiabilityEndBalance = HArr::sumAtDates([$totalRentLiabilityEndBalance,$newBranchMicrofinanceOpeningProjection->rent_liability_statement['end_balance']??[]], $sumKeys);
		});
		$tableDataFormatted[$currentTabIndex]['sub_items']['New Branches Rent Right Of Use']['data']  = $totalNewBranchesRentRightOfUserEndBalance ;
		$tableDataFormatted[$currentTabIndex]['sub_items']['New Branches Rent Right Of Use']['year_total']  = HArr::getPerYearIndexForEndBalance($totalNewBranchesRentRightOfUserEndBalance, $yearWithItsMonths) ;
		$totalOtherDebtorsOpeningBalances = HArr::sumAtDates([$totalOtherDebtorsOpeningBalances,$rentRightOfUse ,$totalNewBranchesRentRightOfUserEndBalance ], $sumKeys);
        ...
        */
		...
		$rentLiability = LeaseRentLiabilityOpeningBalance::where('study_id',$this->id)->first();
		$rentLiability = $rentLiability ? $rentLiability->statement['monthly']['end_balance'] : [];
		$title = __('Existing Branches Rent Liability');
		$tableDataFormatted[$currentTabIndex]['sub_items'][$title]['data']  = $rentLiability ;
		$tableDataFormatted[$currentTabIndex]['sub_items'][$title]['year_total']  = HArr::getPerYearIndexForEndBalance($rentLiability, $yearWithItsMonths) ;
		
		
		
		// $totalRentLiabilityEndBalance = LeaseRentLiabilityOpeningBalance::where('study_id',$this->id)->first();
		// $rentLiability = $rentLiability ? $rentLiability->statement['monthly']['end_balance'] : [];
		$title = __('New Branches Rent Liability');
		$tableDataFormatted[$currentTabIndex]['sub_items'][$title]['data']  = $totalRentLiabilityEndBalance ;
		$tableDataFormatted[$currentTabIndex]['sub_items'][$title]['year_total']  = HArr::getPerYearIndexForEndBalance($totalRentLiabilityEndBalance, $yearWithItsMonths) ;
		
		
		
		
		$totalOtherCreditorsOpeningBalances = HArr::sumAtDates([$totalOtherCreditorsOpeningBalances,$rentLiability,$totalRentLiabilityEndBalance ], $sumKeys);
       ...
       // $totalCorporateTaxes =  $totalCorporateTaxes['monthly']['end_balance']??[] ;
        ...
        foreach ($otherLongTermsOpeningBalanceEndBalances->pluck('statement')->toArray() as $otherLongTermsOpeningBalanceEndBalance) {
			$otherLongTermsOpeningBalanceEndBalance = json_decode($otherLongTermsOpeningBalanceEndBalance, true);
			$otherLongTermsOpeningBalanceEndBalance = $otherLongTermsOpeningBalanceEndBalance['monthly']['end_balance'] ?? [];
        ...
        foreach ($loansOpeningBalanceEndBalances as $row) {
			...
			$loansOpeningBalanceEndBalance = json_decode($loansOpeningBalanceEndBalance, true);
			$loansOpeningBalanceEndBalance = $loansOpeningBalanceEndBalance['monthly']['end_balance'] ?? [];
        ...
        $data = [
            ...
            'monthly_long_term_investments'=>$totalLongTermInvestmentsOpeningBalances,
            'yearly_long_term_investments'=>$totalLongTermInvestmentsOpeningBalancesPerYears,
        ...
        return  [
        'exportRouteName'=>'export.non.banking.balance.sheet',
          'financialYearEndMonthNumber'=>$financialYearEndMonthNumber,
          ...
          'nextButton' => [
                'link'=>route('non.banking.view.results.dashboard', ['company'=>$this->company->id,'study'=>$this->id]),
    ...
    public function recalculateMicrofinanceTotalCasesCounts(string $type):void
    {
        ...
        $monthlyLoanAmounts = [];
        /////////////////
        $this->microfinanceProductSalesProjects->where('type', $type)->each(function (MicrofinanceProductSalesProject $microfinanceProductSalesProject) use (&$currentTotalCases, &$monthlyLoanAmounts, $totalLoanOfficerCases) {
	...
	public function recalculateStatementReportTablesForManpower():void{
		$studyMonthsForViews = $this->getStudyDates();
        $studyMonthsForViews = array_slice($studyMonthsForViews, 0, $this->getViewStudyEndDateAsIndex()+1);
        $sumKeys = array_keys($studyMonthsForViews);
  ...
  //      $totalExpenses = [];
    ...
    public function handleFixedRepeatingExpenses(Request $request, ?array $accumulatedOpeningBranchesCounts = null, $branchId = null)
    {
        ...
        // foreach (['fixed_monthly_repeating_amount'] as $tableId) {
            ...
            foreach ((array)$request->get($tableId) as $tableDataArr) {
                ...
                $isFixedRepeating = isset($tableDataArr['amount']) ;
                ...
                if ($isFixedRepeating) {
                    ...
                    if ($currentAccumulatedOpeningBranchesCounts ) {
    ...
    private function calculateMicrofinanceForType(bool $isPortfolio)
    {
        ...
        $totalEndBalances=[];
        $portfolioLoans = [];
        $bankLendingRateType = GeneralAndReserveAssumption::MICROFINANCE_BANK_LENDING_MARGIN_RATE;
        $dateWithDateIndex = $this->getDateWithDateIndex();
        $dateIndexWithDate = $this->getDateIndexWithDate();
        $totalPortfolioEndBalance = [];
        $operationDates = range($this->getOperationStartDateAsIndex(), $this->getStudyEndDateAsIndex());
        ...
        $microfinanceSalesProjects->each(function (MicrofinanceProductSalesProject $microfinanceProductSalesProject) use ($isPortfolio, &$portfolioLoans, $operationDates, &$totalPortfolioEndBalance, $dateWithDateIndex, $dateIndexWithDate, $eclAndNewPortfolioFundingRates, &$totalInterests, &$totalSchedulePayments, &$totalEndBalances, $productColumnName, $daysCount, $bankLendingRateType) {
            ...
            if ($isPortfolio) {
                ...
                $rates = $this->generalAndReserveAssumption->getBaseRatesPerMonths(GeneralAndReserveAssumption::MICROFINANCE_BANK_LENDING_MARGIN_RATE);
            ...
            foreach ($monthlyPortfolioLoanAmounts as $loanStartDateAsIndex => $monthlyLoanAmount) {
                ...
                $marginRate=  $isPortfolio ? 0 : $this->generalAndReserveAssumption->getBankLendingMarginRatesAtYearOrMonthIndex($yearOrMonthIndex, $bankLendingRateType);
    ...
    private function calculateConsumerfinanceForType(bool $isPortfolio)
    {
        ...
        $microfinanceSalesProjects->each(function (ConsumerfinanceProductSalesProject $consumerfinanceProductSalesProject) use ($isPortfolio, &$portfolioLoans, $operationDates, &$totalPortfolioEndBalance, $dateWithDateIndex, $dateIndexWithDate, $eclAndNewPortfolioFundingRates, &$totalInterests, &$totalSchedulePayments, &$totalEndBalances, $productColumnName) {
            ...
            if ($isPortfolio) {
                ...
                $rates = $this->generalAndReserveAssumption->getBaseRatesPerMonths(GeneralAndReserveAssumption::CONSUMER_BANK_LENDING_MARGIN_RATE);
                
            }
            foreach ($monthlyPortfolioLoanAmounts as $loanStartDateAsIndex => $monthlyLoanAmount) {
                $yearOrMonthIndex = $isMonthlyStudy ? $loanStartDateAsIndex : $this->getYearIndexFromDateIndex($loanStartDateAsIndex);
                $fundingRate = ($eclAndNewPortfolioFundingRates[$yearOrMonthIndex]??0) /100 ;
                $marginRate=  $isPortfolio ? 0 : $this->generalAndReserveAssumption->getBankLendingMarginRatesAtYearOrMonthIndex($yearOrMonthIndex, GeneralAndReserveAssumption::CONSUMER_BANK_LENDING_MARGIN_RATE);
    ...
    {
        return (bool)$this->microfinanceProductSalesProjects->where('type', 'by-branch')->where('branch_id', $branchId)->count();
    }
    public function hasNewBranchFilled():bool
    {
        return (bool)$this->microfinanceProductSalesProjects->where('type', 'new-branches')->count();
    }
    public function getFixedAssetNextRoute():string
    {
        ...
        // }
		return $study->isExistingCompany() ? route('view.opening.balances.for.non.banking',['company'=>$company->id,'study'=>$study->id])  : route('view.non.banking.forecast.income.statement',['company'=>$company->id , 'study'=>$study->id]);
  ...
  expense_per_employee
    ...
    public function getBankLendingMarginRatesFormatted(array $dates)
    {
        ...
        foreach (GeneralAndReserveAssumption::getBankLendingMarginTypes() as $type) {
            ...
            foreach ($dates as $dateIndex=>$dateFormatted) {
                $results[$type][$dateIndex] =$generalAndReserveAssumption ? $generalAndReserveAssumption->getBankLendingMarginRatesAtYearOrMonthIndex($dateIndex, $type) :0;
            }
        }
	...
	protected function calculateMinCash():array 
	{
		...
		if(!$generalAssumption){
			...
			$this->calculateTotalDispersementFrom($sumKeys,$fromDispersementOfRates , $fromDispersementOf , $totalContractAmounts , $totalDispersement);
	...
	protected function calculateTotalDispersementFrom(array $sumKeys,array $fromDispersementOfRates , int $fromDispersementOf, array $totalRevenues , array &$totalDispersement)
	{
		foreach($sumKeys as $dateAsIndex ){
	...
	public function getRightOfUseRent():array 
	{
		return $this->right_of_use_rent?:[];
	}
	public function getRightOfUserRentAmount():float 
	{
		$rightOfUseRent = $this->getRightOfUseRent();
		return $rightOfUseRent['amount']??0;
	}
	...
	{
		$rightOfUseRent = $this->getRightOfUseRent();
		return $rightOfUseRent['annual_increase_rate']??0;
	}
	public function getRightOfUserRentAnnualDiscountRate():float 
	{
		$rightOfUseRent = $this->getRightOfUseRent();
		return $rightOfUseRent['annual_discount_rate']??0;
	}
	public function getNewBranchesRentAmountFrom(int $branchRentEndDateAsIndex ,int $startDateAsIndex , int $count,array $rightOfUseRentAmountsWithAnnualIncreases ):array
	{
		$results = [];
		$index = 0;
		foreach($rightOfUseRentAmountsWithAnnualIncreases as $currentDateAsIndex => $rightOfUseRentAmountsWithAnnualIncrease){
			$results[$currentDateAsIndex] = 0;
			if($currentDateAsIndex >= $startDateAsIndex){
				if($currentDateAsIndex> $branchRentEndDateAsIndex){
					break;
				}
				$results['with_zero_index'][$index] = $rightOfUseRentAmountsWithAnnualIncrease*$count;
				$results['with_actual_index'][$currentDateAsIndex] = $rightOfUseRentAmountsWithAnnualIncrease*$count;
				$index++;
			}
		}
		return $results;	
	}
	public function calculateLiabilitiesRentStatement($studyEndDateAsIndex,$rightOfUseRentStartDateAsIndex,$netPresetValue,$newBranchesRentAmount,$rightOfUseRentAnnualDiscountRate)
	{
		$beginningBalance = $netPresetValue;
		$isFirstLoop = true ;
		$liabilityRentStatement = [];
		for($i = $rightOfUseRentStartDateAsIndex ; $i <= $studyEndDateAsIndex;$i++){
			$interest = $isFirstLoop ? 0 : $rightOfUseRentAnnualDiscountRate*$beginningBalance;
			$currentPaymentAmount = $newBranchesRentAmount[$i];
			$endBalance = $beginningBalance+$interest-$currentPaymentAmount;
			$beginningBalance=  $endBalance;
			$liabilityRentStatement['beginning_balance'][$i] = $beginningBalance ;
			$liabilityRentStatement['interest'][$i] =$interest ;
			$liabilityRentStatement['payment'][$i] =$currentPaymentAmount ;
			$liabilityRentStatement['end_balance'][$i] =$endBalance ;
			$isFirstLoop=false ;
		}
		return $liabilityRentStatement;
	}
	public function calculateRightOfUseStatement(float $netPresetValue,int $rightOfUseDuration,int $rightOfUseStartDateAsIndex ,int $rightOfUseEndDateAsIndex):array 
	{
		
		$rentAmortization =$rightOfUseDuration ?  $netPresetValue / $rightOfUseDuration : 0;
		$beginningBalance = $netPresetValue;
		$rightOfUseStatement = [];
		for($i = $rightOfUseStartDateAsIndex ; $i <= $rightOfUseEndDateAsIndex;$i++){
			$endBalance = $beginningBalance-$rentAmortization;
			$rightOfUseStatement['beginning_balance'][$i] = $beginningBalance;
			$rightOfUseStatement['rent_amortization'][$i] = $rentAmortization;
			$beginningBalance= $endBalance;
			$rightOfUseStatement['end_balance'][$i] =$beginningBalance;
		}
		return $rightOfUseStatement;
		
	}
	public function calculateRightOfUseRentAndStatement()
	{
		$rightOfUseRent = $this->getRightOfUseRent();
		$studyStartDateAsIndex=  0;
		$newBranchesRentInterest = [];
		$newBranchesRentPayments = [];
		$newBranchesRentAmortization = [];
		$rightOfUseRentAmount = $rightOfUseRent['amount']??0;
		$rightOfUseRentAnnualIncreaseRate = $rightOfUseRent['annual_increase_rate']??0;
		$rightOfUserDuration  = $rightOfUseRent['duration']??$this->getDurationInYears()*12;
		$studyEndDateAsIndex = $this->getStudyEndDateAsIndex();
		$rightOfUseRentAmountsWithAnnualIncrease = (new MonthlyFixedRepeatingAmountEquation)->calculate($rightOfUseRentAmount,$studyStartDateAsIndex,$studyEndDateAsIndex+$rightOfUserDuration,'annually',$rightOfUseRentAnnualIncreaseRate,false,0,0); 
		$rightOfUseRentAmountsWithAnnualIncrease = $rightOfUseRentAmountsWithAnnualIncrease['total_after_vat']??[];
		$rightOfUseRentAnnualDiscountRate = ($rightOfUseRent['annual_discount_rate']??0)/100/12;
		foreach($this->newBranchMicrofinanceOpeningProjections as $newBranchMicrofinanceOpeningProjection){
			$rightOfUseRentStartDateAsIndex = $newBranchMicrofinanceOpeningProjection->getStartDate() ;
			$rentEndDateAsIndex = $newBranchMicrofinanceOpeningProjection->getEndDateAsIndex($rightOfUserDuration);
			$count = $newBranchMicrofinanceOpeningProjection->getCounts();
			$newBranchesRentAmountArr = $this->getNewBranchesRentAmountFrom($rentEndDateAsIndex,$rightOfUseRentStartDateAsIndex,$count,$rightOfUseRentAmountsWithAnnualIncrease );
			$newBranchesRentAmountWithZeroIndex =$newBranchesRentAmountArr['with_zero_index']??[];
			$newBranchesRentAmounts = $newBranchesRentAmountArr['with_actual_index']??[];
			$netPresetValue = Finance::npv($rightOfUseRentAnnualDiscountRate, array_values($newBranchesRentAmountWithZeroIndex)) ;
			// $rightOfUserNetPresetValue=[$rightOfUseRentStartDateAsIndex => $netPresetValue];
			$liabilitiesRentStatement = $this->calculateLiabilitiesRentStatement($rentEndDateAsIndex,$rightOfUseRentStartDateAsIndex,$netPresetValue,$newBranchesRentAmounts,$rightOfUseRentAnnualDiscountRate);
			$rightOfUseStatement = $this->calculateRightOfUseStatement($netPresetValue,$rightOfUserDuration,$rightOfUseRentStartDateAsIndex,$rentEndDateAsIndex);
			$newBranchMicrofinanceOpeningProjection->update([
				'rent_liability_statement'=>$liabilitiesRentStatement,
				'right_of_use_statement'=>$rightOfUseStatement
			]);
			$sumKeys = array_keys($this->getStudyDates());
			$newBranchesRentInterest = HArr::sumAtDates([$newBranchesRentInterest,($liabilitiesRentStatement['interest']??[])],$sumKeys);
			$newBranchesRentAmortization = HArr::sumAtDates([$newBranchesRentAmortization,($rightOfUseStatement['rent_amortization']??[])],$sumKeys);
			$newBranchesRentPayments = HArr::sumAtDates([$newBranchesRentPayments,($liabilitiesRentStatement['payment']??[])],$sumKeys);
		}
		
		$this->incomeStatementReport->update([
			'new_branches_rent_interest'=>$newBranchesRentInterest??[],
			'new_branches_rent_amortization'=>$newBranchesRentAmortization??[]
		]);
		$this->cashflowStatementReport->update([
			'new_branches_rent_payments'=>$newBranchesRentPayments??[]
		]);
	}
}
