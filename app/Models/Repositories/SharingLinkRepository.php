...
class SharingLinkRepository
{
    ...
    protected function generateShareableLink($shareableType): string
    {
        $shareableUrl = SHAREABLE_LINKS;
        return Request()->root() . '/' . App()->getLocale() . '/' . $shareableUrl . '/' . $shareableType . '/' . HStr::generateUniqueStringOfLengthTo(30, 'SharingLink', ['link']);
    }

    public function store(Request $request)
    {
        $link = $this->generateShareableLink(HStr::camel2dashed($request->get('shareable_type')));
        return SharingLink::create([
            ...
            'is_active'=>true ,
            'link'=>$link,
            'identifier'=>HStr::getLastWordInString($link, '/')
        ]);
    }
