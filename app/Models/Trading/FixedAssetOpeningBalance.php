...
class FixedAssetOpeningBalance extends Model
{
	...
	public static function booted()
	{
			...
			static::saved(function(self $model){
				$statements = DB::connection(TRADING_CONNECTION_NAME)->table('fixed_asset_opening_balances')->where('study_id',$model->study->id)->pluck('statement')->toArray();
		//		$dateWithDateIndex = $model->study->getDateWithDateIndex();
				...
				DB::connection(TRADING_CONNECTION_NAME)->table('income_statement_reports')->where('study_id',$model->study->id)->update([
