...
use App\Http\Controllers\Tradings\IncomeStatementController;
use App\Models\Trading\Contract;
use App\Models\Trading\Expense;
use App\Models\Trading\GeneralAndReserveAssumption;
...
class Study extends Model
{
        ...
        static::updated(function (self $study) {
            ...
            if ($study->isDirty('company_nature') && $study->isNewCompany()) {
                foreach ([
                    ...
                    DB::connection(TRADING_CONNECTION_NAME)->table($tableName)->where('study_id', $study->id)->delete();
                }
            }
        });
    }
    ...
    //     DB::connection(TRADING_CONNECTION_NAME)->table('revenue_contracts')->where('study_id', $this->id)->where('revenue_type', $revenueType)->delete();
    ...
    //     DB::connection('trading')->table($loanSchedulePaymentTableName)->where('revenue_stream_type', $revenueStreamType)->where('study_id', $studyId)->delete();
    ...
    //     DB::connection('trading')->table($loanSchedulePaymentTableName)->insert($portfolioLoans);
    //     DB::connection('trading')->table('income_statement_reports')->where('study_id', $this->id)->update([
    //         $revenueStreamType.'_revenue'=>json_encode($totalInterests),
    //         $revenueStreamType.'_bank_interest'=>json_encode($totalBankInterests),
    //     ]);

    //     DB::connection('trading')->table('cashflow_statement_reports')->where('study_id', $this->id)->update([
	...
	protected function refreshTotalExpensePaymentsInIncomeStatement()
	{
        ...
        DB::connection('trading')->table('income_statement_reports')->where('study_id', $this->id)->update($totalPerTypes);
        foreach ($totalPerCategory as $columnName => $currentTotalPerCategory) {
            DB::connection(TRADING_CONNECTION_NAME)->table('income_statement_reports')->where('study_id', $this->id)->update([
    ...
    //             DB::connection('trading')->table($loanSchedulePaymentTableName)->where('revenue_stream_type', $revenueStreamType)->where('study_id', $this->id)->delete();
    ...
    //         // ]);
    //         DB::connection(TRADING_CONNECTION_NAME)->table('portfolio_mortgage_revenue_projection_by_categories')->where('id', $portfolioMortgageCategoryId)->update($portfolioPresentValueResult);
    ...
    //         DB::connection(TRADING_CONNECTION_NAME)->table('cashflow_statement_reports')->where('study_id', $this->id)->update([
...
;
    ...
    public function runIncomeStatementIfFromCashflow()
    {
        if (Request()->has('redirect-to-cashflow')) {
            ...
            return route('trading.cash.in.out.flow.result', ['company'=>$this->company->id,'study'=>$this->id]);
        }
    }
    ...
    */
    ...
    public function recalculateFixedAssets(string $fixedAssetType, bool $isSensitivity = false)
    {
        ...
        DB::connection(TRADING_CONNECTION_NAME)->table($loanTableName)->where('fixed_asset_type', $fixedAssetType)->where('study_id', $this->id)->delete();
        ...
        foreach ($fixedAssets as $fixedAsset) {
            ...
            foreach ($studyDates as $currentDateAsIndex) {
                ...
                if (count($loanArr)) {
                    DB::connection(TRADING_CONNECTION_NAME)->table($loanTableName)->insert(HArr::encodeArr($loanArr));
                }
        ...
        $fixedAssetLoanSchedulePaymentsAndInterests = DB::connection(TRADING_CONNECTION_NAME)->table('fixed_assets_loan_schedule_payments')->where('study_id', $this->id)->get(['interestAmount','schedulePayment']);
    ...
    public function getExpensesViewVars():array
    {
        ...
        return [
            ...
            'title'=>__('Expenses'),
            'storeRoute'=>route('trading.store.expenses', ['company'=>$company->id , 'study'=>$this->id]),
            'yearsWithItsMonths' => $this->getOperationDurationPerYearFromIndexes(),
    ...
    public function getCashInOutFlowViewVars()
    {
        ...
        */
        ...
        // $loanSchedulePayments = DB::connection(TRADING_CONNECTION_NAME)->table('loan_schedule_payments')->where('study_id', $this->id)->where('portfolio_loan_type', 'portfolio')->get();
		...
		$tableDataFormatted[0]['sub_items']['Forecasted Tradings Investments Rent Revenues']['data'] = $propertyForecastedRentCollections;
		$tableDataFormatted[0]['sub_items']['Forecasted Tradings Investments Rent Revenues']['year_total'] = HArr::sumPerYearIndex($propertyForecastedRentCollections, $yearWithItsMonths);
		...
		// }
		// $formattedResult = [];
		// ForecastedProperty::getForecastedTradingsCoveragesAmounts($this, $columnName, $formattedResult);
		// foreach($formattedResult as $monthIndex => $val) {
		// 	$tableDataFormatted[0]['sub_items']['Forecasted Tradings Investments Rent Revenues']['data'][$monthIndex] = $val;
		// 	$tableDataFormatted[0]['sub_items']['Forecasted Tradings Investments Rent Revenues']['year_total'] = HArr::sumPerYearIndex($formattedResult, $yearWithItsMonths);
		// }
            
    
       
        
        $otherDebtors = DB::connection(TRADING_CONNECTION_NAME)->table('other_debtors_opening_balances')->where('study_id', $this->id)->pluck('statement', 'name')->toArray();
        ...
        $otherLongTermAssetOpeningBalances = DB::connection(TRADING_CONNECTION_NAME)->table('other_long_term_assets_opening_balances')->where('study_id', $this->id)->pluck('statement', 'name')->toArray();
        ...
        $supplierPayables = DB::connection(TRADING_CONNECTION_NAME)->table('supplier_payable_opening_balances')->where('study_id', $this->id)->first();
        if ($supplierPayables) {
        ...
        // $totalPerType =[];
        // $loanSchedulePayments = DB::connection(TRADING_CONNECTION_NAME)->table('loan_schedule_payments')->where('study_id', $this->id)->where('portfolio_loan_type', 'bank_portfolio')->get();
        // $loanSchedulePaymentPerType = HArr::sumLoanSchedulePerKey($loanSchedulePayments, $sumKeys, 'revenue_stream_type');
        ...
        $expenses = DB::connection(TRADING_CONNECTION_NAME)->table('expenses')->where('study_id', $this->id)->get();
        ...
        $salaryPayments = DB::connection(TRADING_CONNECTION_NAME)->table('manpowers')->where('study_id', $this->id)->pluck('salary_payments')->toArray();
        $salaryTaxAndSocialInsurances = DB::connection(TRADING_CONNECTION_NAME)->table('manpowers')->where('study_id', $this->id)->pluck('tax_and_social_insurance_statement')->toArray();
        ...
        $fixedAssetPayments = DB::connection(TRADING_CONNECTION_NAME)->table('fixed_assets')->where('study_id', $this->id)->pluck('ffe_payment')->toArray();
        ...
        $totalFixedAssetReplacements = [];
        $fixedAssetReplacements = DB::connection(TRADING_CONNECTION_NAME)->table('fixed_assets')->where('study_id', $this->id)->pluck('depreciation_statement')->toArray();
        ...
        $otherCreditors = DB::connection(TRADING_CONNECTION_NAME)->table('other_credits_opening_balances')->where('study_id', $this->id)->pluck('statement', 'name')->toArray();
        ...
        $longTermLoanOpeningBalances = DB::connection(TRADING_CONNECTION_NAME)->table('long_term_loan_opening_balances')->where('study_id', $this->id)->get(['interests','installments','name'])->toArray();
        ...
        $longTermLiabilitiesOpeningBalances = DB::connection(TRADING_CONNECTION_NAME)->table('other_long_term_liabilities_opening_balances')->where('study_id', $this->id)->get(['payload','name'])->toArray();
		...
		$existingTradingsInstallments = $cashflowStatementReport ? json_decode($cashflowStatementReport->existing_Tradings_installments,true) : [];
		$existingTradingsInstallments = $this->convertStringIndexesToDateIndex($existingTradingsInstallments);
        $tableDataFormatted[1]['sub_items'][__('Existing Tradings Installments')]['data'] = $existingTradingsInstallments;
        $tableDataFormatted[1]['sub_items'][__('Existing Tradings Installments')]['year_total'] = HArr::sumPerYearIndex($existingTradingsInstallments, $yearWithItsMonths);
         
		$newTradingsInstallments = $cashflowStatementReport ? json_decode($cashflowStatementReport->new_Tradings_installments,true) : [];
        $tableDataFormatted[1]['sub_items'][__('New Tradings Installments')]['data'] = $newTradingsInstallments;
        $tableDataFormatted[1]['sub_items'][__('New Tradings Installments')]['year_total'] = HArr::sumPerYearIndex($newTradingsInstallments, $yearWithItsMonths);
        ...
        */
        // $hasAtLeastOneOdas = DB::connection(TRADING_CONNECTION_NAME)->table('microfinance_product_sales_projects')->where('funded_by', 'by-odas')->where('study_id', $this->id)->count();
        ...
        //   'hasMicrofinanceWithOdas'=>$hasMicrofinanceWithOdas,
          ...
          'exportRouteName'=>'export.trading.forecast.cash.in.out',
          'nextButton' => [
                'link'=>route('trading.balance.sheet.result', ['company'=>$this->company->id,'study'=>$this->id]),
    ...
    public function getBalanceSheetViewVars()
    {
        ...
        // DB::connection(TRADING_CONNECTION_NAME)->table('');
        ...
        $fixedAssetStatements = DB::connection(TRADING_CONNECTION_NAME)->table('fixed_assets')->where('fixed_assets.study_id', $this->id)->join('fixed_asset_names', 'fixed_asset_names.id', '=', 'fixed_assets.name_id')->get(['depreciation_statement','name as title','name_id']);
        
    
        $fixedAssetOpeningBalancesEndBalances = DB::connection(TRADING_CONNECTION_NAME)->table('fixed_asset_opening_balances')->where('study_id', $this->id)->get(['name_id','statement']);
        ...
        */
         ...
         */
        $totalOtherLongTermAssets = DB::connection(TRADING_CONNECTION_NAME)->table('other_long_term_assets_opening_balances')->where('study_id', $this->id)->pluck('statement')->toArray();
        ...
        $cashAndBankOpeningBalances = DB::connection(TRADING_CONNECTION_NAME)->table('cash_and_bank_opening_balances')->where('study_id', $this->id)->pluck('statement')->toArray();
        $cashAndBankOpeningBalances= HArr::formatMultiSubItems($cashAndBankOpeningBalances, $sumKeys, ['monthly','end_balance']);
        
		$NonPerformingCashAndBankOpeningBalances = DB::connection(TRADING_CONNECTION_NAME)->table('cash_and_bank_opening_balances')->where('study_id', $this->id)->pluck('non_performing_statement')->toArray();
        ...
        $totalLoanBalances = [];
        $loanAccuredInterestsWithEndBalances =  DB::connection(TRADING_CONNECTION_NAME)->table('loan_schedule_payments')->where('portfolio_loan_type', 'portfolio')->where('study_id', $this->id)->get(['endBalance','accured_interest','revenue_stream_type','securitization_date_index','id'])->toArray();
        ...
        // $loanEndBalancesForPortfolioMortgages =  DB::connection(TRADING_CONNECTION_NAME)->table('loan_schedule_payments')->where('portfolio_loan_type', 'portfolio')->where('revenue_stream_type',Study::PORTFOLIO_MORTGAGE)->where('study_id', $this->id)->get(['endBalance','revenue_stream_type'])->toArray();
        ...
        // $loanDirectFactoringEndBalances =  DB::connection(TRADING_CONNECTION_NAME)->table('direct_factoring_breakdowns')->where('study_id', $this->id)->pluck('statement_end_balance')->toArray();
        ...
        // $eclStatement =  DB::connection(TRADING_CONNECTION_NAME)->table('ecl_and_new_portfolio_funding_rates')->where('study_id', $this->id)->pluck('accumulated_ecl_values')->toArray();
        ...
        */
        ...
        $otherDebtorsOpeningBalances = DB::connection(TRADING_CONNECTION_NAME)->table('other_debtors_opening_balances')->where('study_id', $this->id)->pluck('statement', 'name')->toArray();
        ...
        // $accruedInterestRevenues = DB::connection(TRADING_CONNECTION_NAME)->table('loan_schedule_payments')->where('study_id',$this->id)->where('portfolio_loan_type','portfolio')->pluck('accured_interest','revenue_stream_type')->toArray();
        ...
        $expenses = DB::connection(TRADING_CONNECTION_NAME)->table('expenses')->where('relation_name', 'one_time_expense')->where('study_id', $this->id)->get();
        $totalExpensePerCategory = [];
        foreach ($expenses as $expense) {
            $expenseNameId = $expense->expense_name_id;
		...
		$title = 'Existing Tradings Creditors Outstanding';
		$existingTotalDueInstallments = DB::connection(TRADING_CONNECTION_NAME)->table('property_due_installments')->where('company_id', $this->company_id)->pluck('total_due_installments')->toArray();
		foreach($existingTotalDueInstallments as $existingTotalDueInstallment){
			...
			foreach($existingTotalDueInstallment as $dateAsString => $amount){
				...
				if($dateIndex){
					...
					$existingTradingsCreditorsOutstanding[$dateIndex] = isset($existingTradingsCreditorsOutstanding[$dateIndex]) ?  $existingTradingsCreditorsOutstanding[$dateIndex] + $currentAmount : $currentAmount ;
				}
			}
		}
        $tableDataFormatted[$currentTabIndex]['sub_items'][$title]['options']['title'] =$title ;
        $tableDataFormatted[$currentTabIndex]['sub_items'][$title]['data'] =$existingTradingsCreditorsOutstanding ;
        $tableDataFormatted[$currentTabIndex]['sub_items'][$title]['year_total'] =HArr::getPerYearIndexForEndBalance($existingTradingsCreditorsOutstanding, $yearWithItsMonths) ;
        
		$title = 'New Tradings Creditors Outstanding';
		$newTotalDueInstallments = DB::connection(TRADING_CONNECTION_NAME)->table('forecasted_property_due_installments')->where('study_id', $this->id)->pluck('total_due_installments')->toArray();
		foreach($newTotalDueInstallments as $newTotalDueInstallment){
			...
			foreach($newTotalDueInstallment as $dateIndex => $amount){
				...
				$newTradingsCreditorsOutstanding[$dateIndex] = isset($newTradingsCreditorsOutstanding[$dateIndex]) ?  $newTradingsCreditorsOutstanding[$dateIndex] + $currentAmount : $currentAmount ;
			}
		}
        $tableDataFormatted[$currentTabIndex]['sub_items'][$title]['options']['title'] =$title ;
        $tableDataFormatted[$currentTabIndex]['sub_items'][$title]['data'] =$newTradingsCreditorsOutstanding ;
        $tableDataFormatted[$currentTabIndex]['sub_items'][$title]['year_total'] =HArr::getPerYearIndexForEndBalance($newTradingsCreditorsOutstanding, $yearWithItsMonths) ;
        
        $totalLoanBalances = [];
        
        $loanAccuredInterestsExpensesWithEndBalances =  DB::connection(TRADING_CONNECTION_NAME)->table('loan_schedule_payments')->where('portfolio_loan_type', 'bank_portfolio')->where('study_id', $this->id)->get(['endBalance','accured_interest','revenue_stream_type','securitization_date_index'])->toArray();
        ...
        // $loanDirectFactoringEndBalances =  DB::connection(TRADING_CONNECTION_NAME)->table('direct_factoring_breakdowns')->where('study_id', $this->id)->pluck('bank_end_balance')->toArray();
        // $loanDirectFactoringEndBalances = HArr::sumJsonArr(array_values($loanDirectFactoringEndBalances), $sumKeys);
        ...
        $totalBankLoanPayable =  HArr::sumAtDates([$existingTradingsCreditorsOutstanding,$newTradingsCreditorsOutstanding,$totalLoanBalances,$loanDirectFactoringEndBalances], $sumKeys);
        ...
        */
        ...
        $otherCreditorsOpeningBalances = DB::connection(TRADING_CONNECTION_NAME)->table('other_credits_opening_balances')->where('study_id', $this->id)->pluck('statement', 'name')->toArray();
        ...
        $salaryStatements = DB::connection(TRADING_CONNECTION_NAME)->table('manpowers')->where('study_id', $this->id)->pluck('tax_and_social_insurance_statement')->toArray();
        ...
        // $directFactoringUnearnedRevenues = DB::connection(TRADING_CONNECTION_NAME)->table('direct_factoring_breakdowns')->where('study_id', $this->id)->pluck('end_balance')->toArray();
        ...
        // $portfolioFactoringUnearnedRevenues = DB::connection(TRADING_CONNECTION_NAME)->table('portfolio_mortgage_revenue_projection_by_categories')->where('study_id', $this->id)->pluck('portfolio_mortgage_unearned_interest_statement')->toArray();
        ...
        $fixedAssetPayables = DB::connection(TRADING_CONNECTION_NAME)->table('fixed_assets')->where('study_id', $this->id)->pluck('ffe_payable')->toArray();
        ...
        $otherLongTermsOpeningBalanceEndBalances =  DB::connection(TRADING_CONNECTION_NAME)->table('other_long_term_liabilities_opening_balances')->where('study_id', $this->id)->get();
        ...
        $totalLoanSubs=[];
        $totalLoansOpening = [];
        $loansOpeningBalanceEndBalances =  DB::connection(TRADING_CONNECTION_NAME)->table('long_term_loan_opening_balances')->where('study_id', $this->id)->get();
        ...
        return  [
        'exportRouteName'=>'export.trading.balance.sheet',
          'financialYearEndMonthNumber'=>$financialYearEndMonthNumber,
          ...
          'nextButton' => [
                'link'=>route('trading.view.results.dashboard', ['company'=>$this->company->id,'study'=>$this->id]),
    ...
    {
        $corporateTaxesPayable = DB::connection(TRADING_CONNECTION_NAME)->table('vat_and_credit_withhold_tax_opening_balances')->where('study_id', $this->id)->first();
    ...
    //     return route('view.manpower.for.trading', ['company'=>$this->company->id,'study'=>$this->id]);
    ...
    //         }
    //     });
    //     DB::connection('trading')->table('loan_schedule_payments')->insert($portfolioLoans);
    //     return [
    ...
    public function getFixedAssetNextRoute():string
    {
        ...
        // }
		return $study->isExistingCompany() ? route('view.opening.balances.for.trading',['company'=>$company->id,'study'=>$study->id])  : route('view.trading.forecast.income.statement',['company'=>$company->id , 'study'=>$study->id]);
    ...
    //     $sumKeys = array_keys($studyMonthsForViews);
    //     //   $openingBalance = 0 ;
    //     // $odasWithdrawal = DB::connection(TRADING_CONNECTION_NAME)->table('ecl_and_new_portfolio_funding_rates')->where('study_id', $this->id)->where('revenue_stream_type', Study::MICROFINANCE)->first();
    ...
    //     // $fixedAssetStatements = DB::connection(TRADING_CONNECTION_NAME)->table('fixed_assets')->where('fixed_assets.study_id', $this->id)->join('fixed_asset_names', 'fixed_asset_names.id', '=', 'fixed_assets.name_id')->get(['depreciation_statement','name as title'])->toArray();
    //     // $fixedAssetOpeningBalancesEndBalances = DB::connection(TRADING_CONNECTION_NAME)->table('fixed_asset_opening_balances')->where('study_id', $this->id)->pluck('statement')->toArray();
  ...
  expense_per_employee
    ...
    {
        $total = 0;
        $otherDebtorsOpeningBalances = DB::connection(TRADING_CONNECTION_NAME)->table('other_debtors_opening_balances')->where('study_id', $this->id)->pluck('amount', 'name')->toArray();
    ...
    {
        $total = 0;
        $otherCreditorsOpeningBalances = DB::connection(TRADING_CONNECTION_NAME)->table('other_credits_opening_balances')->where('study_id', $this->id)->pluck('amount', 'name')->toArray();
	...
	protected function calculateMinCash():array 
	{
		...
		if(!$generalAssumption){
			...
			//  $resultArr = DB::connection(TRADING_CONNECTION_NAME)->table('revenue_contracts')
	...
	public function getTotalExpenses():array 
	{
			$yearWithItsIndexes = $this->getOperationDurationPerYearFromIndexes();
			        $monthsWithItsYear = $this->getMonthsWithItsYear($yearWithItsIndexes) ;
				$totalExpenses = [];
        $expenses = DB::connection(TRADING_CONNECTION_NAME)->table('expenses')->where('study_id',$this->id)->join('expense_names', 'expense_names.id', '=', 'expenses.expense_name_id')->selectRaw('expenses.expense_category,expense_names.name as name,expenses.relation_name,expenses.monthly_repeating_amounts,expenses.total_after_vat,payload')->where('expenses.model_id', $this->id)->where('expenses.model_name', 'Study')->get()->toArray();
	...
	public function areaTradings():HasMany
	{
	...
	public function removeAreaTradingsForRevenueStreamType(string $revenueStreamType):void
	{
		$this->areaTradings()->where('revenue_stream_type',$revenueStreamType)->delete();
	}
}
