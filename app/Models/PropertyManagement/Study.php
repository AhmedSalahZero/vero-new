...
use App\Equations\ExpenseAsPercentageOfPropertyEquation;
use App\Equations\MonthlyFixedRepeatingAmountEquation;
use App\Equations\OneTimeExpenseEquation;
use App\Helpers\HArr;
use App\Helpers\HHelpers;
use App\Http\Controllers\PropertyManagements\IncomeStatementController;
use App\Models\PropertyManagement\ConsumerfinanceProductSalesProject;
...
class Study extends Model
{
        ...
        static::updated(function (self $study) {
            ...
            if ($study->isDirty('company_nature') && $study->isNewCompany()) {
                foreach ([
                    ...
                    DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table($tableName)->where('study_id', $study->id)->delete();
                }
            }
        });
    }
    ...
    public function getDatesIndexesHelper()
    {
        ...
        // $studyEndDate = Carbon::make(Arr::last($studyDates))->addYears(10)->format('Y-m-d');
		...
		$lastDate = Arr::last($studyDates);
		for($i = 1 ;  $i <= 84 ; $i++){
			$studyDates[] = Carbon::make($lastDate)->addMonths($i)->format('Y-m-d');
		}
    ...
    //     DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('revenue_contracts')->where('study_id', $this->id)->where('revenue_type', $revenueType)->delete();
    ...
    //         foreach ($operationDurationPerYear as $yearIndex => $yearWithItsMonths) {
              
	// 				foreach($yearWithItsMonths as $monthIndex => $trueOrFalse) {
	// 					if ($isDirectFactoring) {
    //                     $directFactoringMonthlyLoanAmounts =   $this->getTotalDirectFactoringNewPortfolioAmountsAtYearOrMonthIndex($monthIndex)['per_category'][$leasingRevenueStreamBreakdownId]??[];
    //                 }
    //                 $yearIndexWithAmount = is_string($yearIndexWithAmount) ? (array)json_decode($yearIndexWithAmount) : $yearIndexWithAmount;
    //                 $loanAtCurrentYear =  ($yearIndexWithAmount[$monthIndex]??0) ;
                   
    //                 $currentMonthlyLoanAmount = $isPortfolio ? 0 :  $loanAtCurrentYear   ;
	...
	// 		$currentMonthlyAmounts = $this->extendEmptyArrayToTheEndOfStudy($currentMonthlyAmounts);
    //         $model->update([
    ...
    // }
	protected function extendEmptyArrayToTheEndOfStudy(array $currentMonthlyAmounts):array 
	{
		if(!count($currentMonthlyAmounts)){
			return [];
		}
		    $studyDurationPerYear=$this->getStudyDurationPerYearFromIndexes();
			$lastExistingDate = array_key_last($currentMonthlyAmounts);
			$endOfStudyDateAsIndex = array_key_last($studyDurationPerYear);
			for($i = $lastExistingDate+1 ; $i <= $endOfStudyDateAsIndex ; $i++){
				$currentMonthlyAmounts[$i] = $currentMonthlyAmounts[$lastExistingDate]??0;
			}
			return $currentMonthlyAmounts;
    ...
    //     $study = $this ;
    //     $bankLendingRateType = $study->getBankLendingRateTypeForRevenueStream($revenueStreamType);
    //     $loanSchedulePaymentTableName = $isSensitivity ? 'sensitivity_loan_schedule_payments' : 'loan_schedule_payments';
    //     DB::connection('property_management')->table($loanSchedulePaymentTableName)->where('revenue_stream_type', $revenueStreamType)->where('study_id', $studyId)->delete();
    ...
    //     // $baseRatesMapping2 = HArr::isAllValuesEqual($baseRatesMapping, $bankLendingMarginRates);
    //     $baseRatesMapping = $this->generalAndReserveAssumption->getBaseRatesPerMonths();
    ...
    //                     $newLoanFundingRate = $eclAndNewPortfolioFundingRate->getNewLoansFundingRatesAtYearOrMonthIndex($monthIndex);
    //                     $currentMarginRate = $generalAndReserveAssumption->getBankLendingMarginRatesAtYearOrMonthIndex($monthIndex);
    ...
    //     DB::connection('property_management')->table($loanSchedulePaymentTableName)->insert($portfolioLoans);
    //     DB::connection('property_management')->table('income_statement_reports')->where('study_id', $this->id)->update([
    ...
    public function recalculateExpenses(string $modelName, int $modelId, string $expenseType, array $expenseTypes, $isSensitivity=false)
    {
        ...
        $expenseAsPercentageOfPropertyEquation= new ExpenseAsPercentageOfPropertyEquation ;
		...
		$this->refreshTotalExpensePaymentsInIncomeStatement();
	...
	protected function refreshTotalExpensePaymentsInIncomeStatement()
	{
        ...
        DB::connection('property_management')->table('income_statement_reports')->where('study_id', $this->id)->update($totalPerTypes);
        foreach ($totalPerCategory as $columnName => $currentTotalPerCategory) {
            DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('income_statement_reports')->where('study_id', $this->id)->update([
    ...
    // public function recalculatePortfolioMortgage($request,$isSensitivity=false)
    // {
    //     $loanSchedulePaymentTableName = $isSensitivity ? 'sensitivity_loan_schedule_payments':'loan_schedule_payments';
    //     $revenueStreamType = Study::PORTFOLIO_MORTGAGE;
    //             DB::connection('property_management')->table($loanSchedulePaymentTableName)->where('revenue_stream_type', $revenueStreamType)->where('study_id', $this->id)->delete();
    ...
    //         // ]);
    //         DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('portfolio_mortgage_revenue_projection_by_categories')->where('id', $portfolioMortgageCategoryId)->update($portfolioPresentValueResult);
    ...
    //         DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('cashflow_statement_reports')->where('study_id', $this->id)->update([
    ...
    public function calculateCorporateTaxesFromIncomeStatementReport(IncomeStatementReport $incomeStatementReport, array $yearWithItsMonths, int $startDateAsIndex, int $endDateAsIndex, array $dateIndexWithDate)
    {
        ...
        $expenses = [
            ...
            // 'non_performing_existing_ecl_expenses',
            // 'ecl_expenses',
    ...
    public function recalculateCashflowStatement()
    {
     ...
     //   $this->calculateSecuritizationLoans();
		...
		$isFirstLoop = true ;
		$firstDateAsIndex = null;
        foreach ($studyDates as $dateAsIndex => $dateAsString) {
			
			$minCash = $minCashes[$dateAsIndex]??0 ;
			if($isFirstLoop){
				// $firstDateAsIndex = $dateAsIndex;
			}else{
				// $totalMinCashBeforeMe =array_sum(array_slice($minCashes,$firstDateAsIndex,$dateAsIndex)) ;
				// $minCash =  $minCash ;
			}
			$isFirstLoop =false;
            ...
            $beforeSettlement = $odaOpeningBalance + $currentWithdrawalAmount ;
             $currentSettlement = $currentNetCashBeforeWorkingCapital - $minCash + $currentWithdrawalAmount ;
			  if ($currentNetCashBeforeWorkingCapital+$currentWithdrawalAmount < 0 || $currentSettlement < 0 ) {
            ...
            if ($netCashBeforeExtraCapitalAndAfterOdaInjection<= 0) {
                $result['extra_capital_injection'][$dateAsIndex] = ($netCashBeforeExtraCapitalAndAfterOdaInjection*-1)+$minCash;
			}elseif($netCashBeforeExtraCapitalAndAfterOdaInjection>0 && $netCashBeforeExtraCapitalAndAfterOdaInjection< $minCash) {
				$result['extra_capital_injection'][$dateAsIndex] = $minCash - $netCashBeforeExtraCapitalAndAfterOdaInjection;
			}
            ...
            $result['oda_statements']['oda_interests'][$dateAsIndex] =   $interest ;
						...
						$result['oda_statements']['extra_odas_withdrawal'][$dateAsIndex] = $result['extra_capital_injection'][$dateAsIndex] ;
			// $result['oda_statements']['extra_odas_withdrawal_interest'][$dateAsIndex] = $result['extra_capital_injection'][$dateAsIndex] ;
			
            $result['oda_statements']['end_balance'][$dateAsIndex] =   $interest + $currentTotalDues +$result['oda_statements']['extra_odas_withdrawal'][$dateAsIndex] = $result['extra_capital_injection'][$dateAsIndex] ;
;
...
;
			...
			// $currentWithdrawalAmount = $currentWithdrawalAmount + $result['extra_capital_injection'][$dateAsIndex];
			// $result['extra_capital_injection'][$dateAsIndex] = 0;
    ...
    public function runIncomeStatementIfFromCashflow()
    {
        if (Request()->has('redirect-to-cashflow')) {
            ...
            return route('property.management.cash.in.out.flow.result', ['company'=>$this->company->id,'study'=>$this->id]);
        }
    }
    ...
    //                 $currentMonthlyLoanAmount = $loanArr['loan_amounts'][$monthIndex] ;
    //                 if ($currentMonthlyLoanAmount <= 0) {
    //                     continue ;
    //                 }
    ...
    */
    ...
    public function recalculateFixedAssets(string $fixedAssetType, bool $isSensitivity = false)
    {
        ...
        DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table($loanTableName)->where('fixed_asset_type', $fixedAssetType)->where('study_id', $this->id)->delete();
        ...
        foreach ($fixedAssets as $fixedAsset) {
            ...
            foreach ($studyDates as $currentDateAsIndex) {
                ...
                if (count($loanArr)) {
                    DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table($loanTableName)->insert(HArr::encodeArr($loanArr));
                }
        ...
        $fixedAssetLoanSchedulePaymentsAndInterests = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('fixed_assets_loan_schedule_payments')->where('study_id', $this->id)->get(['interestAmount','schedulePayment']);
    ...
    public function getDateIndexFromString(string $dateAsString):?int
    {
        $dateWithDateIndex = $this->getDateWithDateIndex();
        return $dateWithDateIndex[$dateAsString]??null;
    }
    public function getIndexDateFromString(string $dateAsString):?int
    {
        return $this->getDateIndexFromString($dateAsString);
    }
    ...
    public function getCashInOutFlowViewVars()
    {
        ...
        */
        ...
        // $loanSchedulePayments = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('loan_schedule_payments')->where('study_id', $this->id)->where('portfolio_loan_type', 'portfolio')->get();
		...
		$tableDataFormatted[0]['sub_items']['Property Contract Full Rent Coverage Contracts']['year_total'] = HArr::sumPerYearIndex($fullCoverageRentCollections, $yearWithItsMonths);
		$tableDataFormatted[0]['sub_items']['Property Contract Partial Rent Coverage Contracts']['data'] = $partialCoverageRentCollections;
		$tableDataFormatted[0]['sub_items']['Property Contract Partial Rent Coverage Contracts']['year_total'] = HArr::sumPerYearIndex($partialCoverageRentCollections, $yearWithItsMonths);
		$tableDataFormatted[0]['sub_items']['Property To Be Delivered Rent Revenues']['data'] = $toBeDeliveredRentCollections;
		$tableDataFormatted[0]['sub_items']['Property To Be Delivered Rent Revenues']['year_total'] = HArr::sumPerYearIndex($toBeDeliveredRentCollections, $yearWithItsMonths);
		$tableDataFormatted[0]['sub_items']['Forecasted Properties Investments Rent Revenues']['data'] = $propertyForecastedRentCollections;
		$tableDataFormatted[0]['sub_items']['Forecasted Properties Investments Rent Revenues']['year_total'] = HArr::sumPerYearIndex($propertyForecastedRentCollections, $yearWithItsMonths);
		// $columnName = 'rent_collections';
		// $formattedResult = [];
		// PropertyContractFullRentRenewal::getFullRentCoveragesAmounts($this, $columnName, $formattedResult);
		// foreach($formattedResult as $monthIndex => $val) {
		// 	$tableDataFormatted[0]['sub_items']['Property Contract Full Rent Coverage Contracts']['data'][$monthIndex] = $val;
		// 	$tableDataFormatted[0]['sub_items']['Property Contract Full Rent Coverage Contracts']['year_total'] = HArr::sumPerYearIndex($formattedResult, $yearWithItsMonths);
		// }
		// $formattedResult = [];
		// PropertyContractPartialRentRenewal::getPartialRentCoveragesAmounts($this, $columnName, $formattedResult);
		// foreach($formattedResult as $monthIndex => $val) {
		// 	$tableDataFormatted[0]['sub_items']['Property Contract Partial Rent Coverage Contracts']['data'][$monthIndex] = $val;
		// 	$tableDataFormatted[0]['sub_items']['Property Contract Partial Rent Coverage Contracts']['year_total'] = HArr::sumPerYearIndex($formattedResult, $yearWithItsMonths);
		// }
		// $formattedResult = [];
		// PropertyToBeDelivered::getToBeDeliveredCoveragesAmounts($this, $columnName, $formattedResult);
		// foreach($formattedResult as $monthIndex => $val) {
		// 	$tableDataFormatted[0]['sub_items']['Property To Be Delivered Rent Revenues']['data'][$monthIndex] = $val;
		// 	$tableDataFormatted[0]['sub_items']['Property To Be Delivered Rent Revenues']['year_total'] = HArr::sumPerYearIndex($formattedResult, $yearWithItsMonths);
		// }
		// $formattedResult = [];
		// ForecastedProperty::getForecastedPropertiesCoveragesAmounts($this, $columnName, $formattedResult);
		// foreach($formattedResult as $monthIndex => $val) {
		// 	$tableDataFormatted[0]['sub_items']['Forecasted Properties Investments Rent Revenues']['data'][$monthIndex] = $val;
		// 	$tableDataFormatted[0]['sub_items']['Forecasted Properties Investments Rent Revenues']['year_total'] = HArr::sumPerYearIndex($formattedResult, $yearWithItsMonths);
		// }
            
    
       
        
        $otherDebtors = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('other_debtors_opening_balances')->where('study_id', $this->id)->pluck('statement', 'name')->toArray();
        ...
        $otherLongTermAssetOpeningBalances = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('other_long_term_assets_opening_balances')->where('study_id', $this->id)->pluck('statement', 'name')->toArray();
        ...
        $supplierPayables = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('supplier_payable_opening_balances')->where('study_id', $this->id)->first();
        if ($supplierPayables) {
		...
		//$existingPortfolioCorridorChangesInterestTitle =__('Existing Portfolio Corridor Changes ( (Gain) / Loss )');
		// $loansExistingPortfolioCorridorChangesInterest = $incomeStatementReport->loans_interest_corridor_changes?:[];
		// $tableDataFormatted[1]['sub_items'][$existingPortfolioCorridorChangesInterestTitle]['options']['title'] = $existingPortfolioCorridorChangesInterestTitle;
		// $tableDataFormatted[1]['sub_items'][$existingPortfolioCorridorChangesInterestTitle]['data']= $loansExistingPortfolioCorridorChangesInterest;
        ...
        // $totalPerType =[];
        // $loanSchedulePayments = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('loan_schedule_payments')->where('study_id', $this->id)->where('portfolio_loan_type', 'bank_portfolio')->get();
        // $loanSchedulePaymentPerType = HArr::sumLoanSchedulePerKey($loanSchedulePayments, $sumKeys, 'revenue_stream_type');
        ...
        $expenses = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('expenses')->where('study_id', $this->id)->get();
        ...
        $salaryPayments = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('manpowers')->where('study_id', $this->id)->pluck('salary_payments')->toArray();
        $salaryTaxAndSocialInsurances = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('manpowers')->where('study_id', $this->id)->pluck('tax_and_social_insurance_statement')->toArray();
        ...
        $corporateTaxesPayments = $this->incomeStatement ?  HArr::array_get($this->incomeStatement->monthly_corporate_taxes_statements, 'monthly.payment') : [];
        ...
        $tableDataFormatted[1]['sub_items'][__('Corporate Taxes Payment')]['year_total'] = HArr::sumPerYearIndex($corporateTaxesPayments, $yearWithItsMonths);
		  ...
		  $dividendPayments =$this->cashflowStatementReport ? $this->cashflowStatementReport->opening_dividend_payments : [];
		  $dividendPayments= $dividendPayments?:[];
			$tableDataFormatted[1]['sub_items'][__('Dividend Payment')]['data'] = $dividendPayments;
			$tableDataFormatted[1]['sub_items'][__('Dividend Payment')]['year_total'] = HArr::sumPerYearIndex($dividendPayments, $yearWithItsMonths);
        ...
        $fixedAssetPayments = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('fixed_assets')->where('study_id', $this->id)->pluck('ffe_payment')->toArray();
        ...
        $totalFixedAssetReplacements = [];
        $fixedAssetReplacements = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('fixed_assets')->where('study_id', $this->id)->pluck('depreciation_statement')->toArray();
        ...
        $otherCreditors = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('other_credits_opening_balances')->where('study_id', $this->id)->pluck('statement', 'name')->toArray();
        ...
        $longTermLoanOpeningBalances = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('long_term_loan_opening_balances')->where('study_id', $this->id)->get(['interests','installments','name'])->toArray();
        ...
        $longTermLiabilitiesOpeningBalances = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('other_long_term_liabilities_opening_balances')->where('study_id', $this->id)->get(['payload','name'])->toArray();
        ...
        */
        ...
        $tableDataFormatted[$currentTabIndex]['main_items'][$currentTabId]['options'] = array_merge([
           'title'=>__('Calculated Extra Equity Injection')
        ], $defaultNumericInputClasses);
        ...
        */
        // $hasAtLeastOneOdas = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('microfinance_product_sales_projects')->where('funded_by', 'by-odas')->where('study_id', $this->id)->count();
        ...
        //   'hasMicrofinanceWithOdas'=>$hasMicrofinanceWithOdas,
          ...
          'exportRouteName'=>'export.property.management.forecast.cash.in.out',
          'nextButton' => [
                'link'=>route('property.management.balance.sheet.result', ['company'=>$this->company->id,'study'=>$this->id]),
    ...
    public function getBalanceSheetViewVars()
    {
        ...
        // DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('');
        ...
        $fixedAssetStatements = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('fixed_assets')->where('fixed_assets.study_id', $this->id)->join('fixed_asset_names', 'fixed_asset_names.id', '=', 'fixed_assets.name_id')->get(['depreciation_statement','name as title','name_id']);
        
    
        $fixedAssetOpeningBalancesEndBalances = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('fixed_asset_opening_balances')->where('study_id', $this->id)->get(['name_id','statement']);
        ...
        */
         ...
         */
        $totalOtherLongTermAssets = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('other_long_term_assets_opening_balances')->where('study_id', $this->id)->pluck('statement')->toArray();
        ...
        $cashAndBankOpeningBalances = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('cash_and_bank_opening_balances')->where('study_id', $this->id)->pluck('statement')->toArray();
        $cashAndBankOpeningBalances= HArr::formatMultiSubItems($cashAndBankOpeningBalances, $sumKeys, ['monthly','end_balance']);
        
		$NonPerformingCashAndBankOpeningBalances = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('cash_and_bank_opening_balances')->where('study_id', $this->id)->pluck('non_performing_statement')->toArray();
        $NonPerformingCashAndBankOpeningBalances= HArr::formatMultiSubItems($cashAndBankOpeningBalances, $sumKeys, ['monthly','end_balance']);
		$cashAndBankOpeningBalances = HArr::sumAtDates([$cashAndBankOpeningBalances,$NonPerformingCashAndBankOpeningBalances], $sumKeys);
        ...
        $totalLoanBalances = [];
        $loanAccuredInterestsWithEndBalances =  DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('loan_schedule_payments')->where('portfolio_loan_type', 'portfolio')->where('study_id', $this->id)->get(['endBalance','accured_interest','revenue_stream_type','securitization_date_index','id'])->toArray();
        ...
        // $loanEndBalancesForPortfolioMortgages =  DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('loan_schedule_payments')->where('portfolio_loan_type', 'portfolio')->where('revenue_stream_type',Study::PORTFOLIO_MORTGAGE)->where('study_id', $this->id)->get(['endBalance','revenue_stream_type'])->toArray();
        ...
        // $loanDirectFactoringEndBalances =  DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('direct_factoring_breakdowns')->where('study_id', $this->id)->pluck('statement_end_balance')->toArray();
        ...
        // $eclStatement =  DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('ecl_and_new_portfolio_funding_rates')->where('study_id', $this->id)->pluck('accumulated_ecl_values')->toArray();
		...
		$loanDirectFactoringEndBalances  =[];
        ...
        $totalCustomerReceivables = HArr::sumAtDates([$cashAndBankOpeningBalances,$totalLoanBalances,$loanDirectFactoringEndBalances], $sumKeys);
        ...
        */
        ...
        $otherDebtorsOpeningBalances = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('other_debtors_opening_balances')->where('study_id', $this->id)->pluck('statement', 'name')->toArray();
        ...
        // $accruedInterestRevenues = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('loan_schedule_payments')->where('study_id',$this->id)->where('portfolio_loan_type','portfolio')->pluck('accured_interest','revenue_stream_type')->toArray();
        ...
        $expenses = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('expenses')->where('relation_name', 'one_time_expense')->where('study_id', $this->id)->get();
        $totalExpensePerCategory = [];
        foreach ($expenses as $expense) {
            $expenseNameId = $expense->expense_name_id;
		...
		// $tableDataFormatted[$currentTabIndex]['sub_items'][$title]['options']['title'] =$title ;
		$tableDataFormatted[$currentTabIndex]['main_items'][$currentTabIndex]['options']['title'] = __('ODAs Outstanding');
		...
		foreach($existingTotalDueInstallments as $existingTotalDueInstallment){
			$existingTotalDueInstallment = json_decode($existingTotalDueInstallment,true);
			foreach($existingTotalDueInstallment as $dateAsString => $amount){
				$dateIndex = $this->getIndexDateFromString($dateAsString);
				if($dateIndex){
					$currentAmount = HArr::sumStartingFromIndexToTheEnd($existingTotalDueInstallment,$dateIndex);
					$existingPropertiesCreditorsOutstanding[$dateIndex] = isset($existingPropertiesCreditorsOutstanding[$dateIndex]) ?  $existingPropertiesCreditorsOutstanding[$dateIndex] + $currentAmount : $currentAmount ;
				}
			}
		}
        $tableDataFormatted[$currentTabIndex]['sub_items'][$title]['options']['title'] =$title ;
		...
		foreach($newTotalDueInstallments as $newTotalDueInstallment){
			$newTotalDueInstallment = json_decode($newTotalDueInstallment,true);
			foreach($newTotalDueInstallment as $dateIndex => $amount){
				$currentAmount = HArr::sumStartingFromIndexToTheEnd($newTotalDueInstallment,$dateIndex);
				$newPropertiesCreditorsOutstanding[$dateIndex] = isset($newPropertiesCreditorsOutstanding[$dateIndex]) ?  $newPropertiesCreditorsOutstanding[$dateIndex] + $currentAmount : $currentAmount ;
			}
		}
        $tableDataFormatted[$currentTabIndex]['sub_items'][$title]['options']['title'] =$title ;
        $tableDataFormatted[$currentTabIndex]['sub_items'][$title]['data'] =$newPropertiesCreditorsOutstanding ;
        $tableDataFormatted[$currentTabIndex]['sub_items'][$title]['year_total'] =HArr::getPerYearIndexForEndBalance($newPropertiesCreditorsOutstanding, $yearWithItsMonths) ;
        
        $totalLoanBalances = [];
        
        $loanAccuredInterestsExpensesWithEndBalances =  DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('loan_schedule_payments')->where('portfolio_loan_type', 'bank_portfolio')->where('study_id', $this->id)->get(['endBalance','accured_interest','revenue_stream_type','securitization_date_index'])->toArray();
        ...
        // $loanDirectFactoringEndBalances =  DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('direct_factoring_breakdowns')->where('study_id', $this->id)->pluck('bank_end_balance')->toArray();
        // $loanDirectFactoringEndBalances = HArr::sumJsonArr(array_values($loanDirectFactoringEndBalances), $sumKeys);
        ...
        $totalBankLoanPayable =  HArr::sumAtDates([$existingPropertiesCreditorsOutstanding,$newPropertiesCreditorsOutstanding,$totalLoanBalances,$loanDirectFactoringEndBalances], $sumKeys);
        ...
        */
        ...
        $otherCreditorsOpeningBalances = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('other_credits_opening_balances')->where('study_id', $this->id)->pluck('statement', 'name')->toArray();
        ...
        ///////////////
		$totalDirectFactoringUnearnedRevenues = [];
        // $directFactoringUnearnedRevenues = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('direct_factoring_breakdowns')->where('study_id', $this->id)->pluck('end_balance')->toArray();
        ...
        // $portfolioFactoringUnearnedRevenues = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('portfolio_mortgage_revenue_projection_by_categories')->where('study_id', $this->id)->pluck('portfolio_mortgage_unearned_interest_statement')->toArray();
        ...
        $fixedAssetPayables = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('fixed_assets')->where('study_id', $this->id)->pluck('ffe_payable')->toArray();
		...
		$title = __('Dividends Payable');
		$equityOpeningBalance = $this->equityOpeningBalances->first();
		$equityOpeningBalance = $equityOpeningBalance ? $equityOpeningBalance->dividend_statement:[];
		$dividendPayable = $equityOpeningBalance ? $equityOpeningBalance['monthly']['end_balance']:[];

        $tableDataFormatted[$currentTabIndex]['sub_items'][$title]['options']['title'] = $title ;
        $tableDataFormatted[$currentTabIndex]['sub_items'][$title]['data'] = $dividendPayable ;
        $tableDataFormatted[$currentTabIndex]['sub_items'][$title]['year_total'] = HArr::getPerYearIndexForEndBalance($dividendPayable, $yearWithItsMonths) ;
        ...
        $totalExistOtherCreditors = HArr::sumAtDates([$totalOtherCreditorsOpeningBalances,$totalAccruedInterestExpenses,$totalExpenses,$totalWithholdEndBalancePerCategories,$totalCorporateTaxes,$salaryStatementEndBalances,$totalDirectFactoringUnearnedRevenues,$portfolioEndBalances,$totalFixedAssetPayables,$dividendPayable], $sumKeys);
        ...
        $totalLoanSubs=[];
        $totalLoansOpening = [];
        $loansOpeningBalanceEndBalances =  DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('long_term_loan_opening_balances')->where('study_id', $this->id)->get();
        ...
        $additionalPaidUpCapital = HArr::accumulateArray(HArr::sumAtDates([$manualEquityInjections], $sumKeys)) ;
		$additionalPaidUpCapital = HArr::sumAtDates([$additionalPaidUpCapital,$odaEndBalances], $sumKeys);
        ...
        return  [
        'exportRouteName'=>'export.property.management.balance.sheet',
          'financialYearEndMonthNumber'=>$financialYearEndMonthNumber,
          ...
          'nextButton' => [
                'link'=>route('property.management.view.results.dashboard', ['company'=>$this->company->id,'study'=>$this->id]),
    ...
    {
        $corporateTaxesPayable = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('vat_and_credit_withhold_tax_opening_balances')->where('study_id', $this->id)->first();
    ...
    //     return route('view.manpower.for.property.management', ['company'=>$this->company->id,'study'=>$this->id]);
    ...
    //         }
    //     });
    //     DB::connection('property_management')->table('loan_schedule_payments')->insert($portfolioLoans);
    //     return [
    ...
    public function getFixedAssetNextRoute():string
    {
        ...
        // }
		return $study->isExistingCompany() ? route('view.opening.balances.for.property.management',['company'=>$company->id,'study'=>$study->id])  : route('view.property.management.forecast.income.statement',['company'=>$company->id , 'study'=>$study->id]);
    ...
    //     $sumKeys = array_keys($studyMonthsForViews);
    //     //   $openingBalance = 0 ;
    //     // $odasWithdrawal = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('ecl_and_new_portfolio_funding_rates')->where('study_id', $this->id)->where('revenue_stream_type', Study::MICROFINANCE)->first();
    ...
    //     // if($this->hasMicroFinance()||$this->hasConsumerFinance()){
	// 	// 	++$currentTabIndex ;
    //     // $odaWithdrawals = $odaStatements['oda_withdrawals']??[];
    //     // $tableDataFormatted[$currentTabIndex]['main_items'][$currentTabIndex]['options']['title'] = __('Disbursement ODAs Withdrawals');
	...
	// 	// 		  ++$currentTabIndex ;
    //     // $extraOdaWithdrawals = $odaStatements['extra_odas_withdrawal']??[];
    //     // $tableDataFormatted[$currentTabIndex]['main_items'][$currentTabIndex]['options']['title'] = __('Extra ODAs Withdrawal');
    //     // $tableDataFormatted[$currentTabIndex]['main_items'][$currentTabIndex]['data'] = $extraOdaWithdrawals;
    //     // $tableDataFormatted[$currentTabIndex]['main_items'][$currentTabIndex]['year_total'] =  HArr::sumPerYearIndex($extraOdaWithdrawals, $yearWithItsMonths);
        
	// 	// ++$currentTabIndex ;
    //     // $extraOdaWithdrawalsInterests = $odaStatements['extra_odas_withdrawal_interest']??[];
    //     // $tableDataFormatted[$currentTabIndex]['main_items'][$currentTabIndex]['options']['title'] = __('Extra ODAs Withdrawal Interest');
    //     // $tableDataFormatted[$currentTabIndex]['main_items'][$currentTabIndex]['data'] = $extraOdaWithdrawalsInterests;
    //     // $tableDataFormatted[$currentTabIndex]['main_items'][$currentTabIndex]['year_total'] =  HArr::sumPerYearIndex($extraOdaWithdrawalsInterests, $yearWithItsMonths);
    ...
    //     // $fixedAssetStatements = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('fixed_assets')->where('fixed_assets.study_id', $this->id)->join('fixed_asset_names', 'fixed_asset_names.id', '=', 'fixed_assets.name_id')->get(['depreciation_statement','name as title'])->toArray();
    //     // $fixedAssetOpeningBalancesEndBalances = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('fixed_asset_opening_balances')->where('study_id', $this->id)->pluck('statement')->toArray();
  ...
  expense_per_employee
    ...
    {
        $total = 0;
        $otherDebtorsOpeningBalances = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('other_debtors_opening_balances')->where('study_id', $this->id)->pluck('amount', 'name')->toArray();
    ...
    {
        $total = 0;
        $otherCreditorsOpeningBalances = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('other_credits_opening_balances')->where('study_id', $this->id)->pluck('amount', 'name')->toArray();
    ...
    public function getManpowerFormattedForType($departments, array $dates, string $manpowerType, $branchId)
    {
        $results  = [];
        $study  = $this;
        foreach ($departments as $department) {
            foreach ($department->positions as $position) {
                $currentManpower = $position->manpowers->where('type', $manpowerType)->where('study_id', $study->id)->where('branch_id', $branchId)->first();
                $results[$position->id]= [
                    'type'=>$manpowerType,
                    ...
                    'company_id'=>$this->company->id,
                    'study_id'=>$this->id,
                    'existing_count'=>$currentManpower ? $currentManpower->getExistingCount() : 0,
                    'monthly_net_salary'=>$currentManpower ? $currentManpower->getMonthlyNetSalary() : 0,
                    'hiring_counts'=>$currentManpower ? $currentManpower->getHiringCountsFormatted($dates) : array_fill_keys(array_keys($dates), 0)
                ];
            }
        }
        return $results;
    }
	//   public function getCbeBaseLendingCorridorRatesFormatted(array $dates)
    // {
    //     $results=[];
    //     $generalAndReserveAssumption = $this->generalAndReserveAssumption;
    //     foreach ($dates as $dateIndex=>$dateFormatted) {
    //         $results[$dateIndex] =$generalAndReserveAssumption ? $generalAndReserveAssumption->getCbeBaseLendingCorridorRatesAtYearOrMonthIndex($dateIndex) :0;
    //     }
    //     return $results;
    // }  
    public function getCbeLendingCorridorRatesFormatted(array $dates)
    {
        $results=[];
        $generalAndReserveAssumption = $this->generalAndReserveAssumption;
        foreach ($dates as $dateIndex=>$dateFormatted) {
            $results[$dateIndex] =$generalAndReserveAssumption ? $generalAndReserveAssumption->getCbeLendingCorridorRatesAtYearOrMonthIndex($dateIndex) :0;
        }
        return $results;
    }  
	//   public function getCbeCorridorChangesRatesFormatted(array $dates)
    // {
    //     $results=[];
    //     $generalAndReserveAssumption = $this->generalAndReserveAssumption;
    //     foreach ($dates as $dateIndex=>$dateFormatted) {
    //         $results[$dateIndex] =$generalAndReserveAssumption ? $generalAndReserveAssumption->getCbeCorridorChangesRatesAtYearOrMonthIndex($dateIndex) :0;
    //     }
    //     return $results;
    // }
    public function getCreditInterestRateForSurplusCashesFormatted(array $dates)
    {
        $results = [];
        $generalAndReserveAssumption = $this->generalAndReserveAssumption;
        foreach ($dates as $dateIndex=>$dateFormatted) {
            $results[$dateIndex] =$generalAndReserveAssumption ? $generalAndReserveAssumption->getCreditInterestRateForSurplusCashAtYearOrMonthIndex($dateIndex) :0;
        }
        return $results;
    }
    public function getBankLendingMarginRatesFormatted(array $dates)
    {
        $results=[];
        $generalAndReserveAssumption = $this->generalAndReserveAssumption;
        // foreach (GeneralAndReserveAssumption::getBankLendingMarginTypes() as $type) {
           
            foreach ($dates as $dateIndex=>$dateFormatted) {
                $results[$dateIndex] =$generalAndReserveAssumption ? $generalAndReserveAssumption->getBankLendingMarginRatesAtYearOrMonthIndex($dateIndex) :0;
            }
        // }
        
        return $results;
    }
    public function getOdasBankLendingMarginRatesFormatted(array $dates):array
    {
        $results=[];
        $generalAndReserveAssumption = $this->generalAndReserveAssumption;
        foreach ($dates as $dateIndex=>$dateFormatted) {
            $results[$dateIndex] =$generalAndReserveAssumption ? $generalAndReserveAssumption->getOdasBankLendingMarginRatesAtYearOrMonthIndex($dateIndex) :0;
        }
        return $results;
    }
	// public function getFromDispersementOfRates(array $dates):array
    // {
    //     $results=[];
    //     $generalAndReserveAssumption = $this->generalAndReserveAssumption;
    //     foreach ($dates as $dateIndex=>$dateFormatted) {
    //         $results[$dateIndex] =$generalAndReserveAssumption ? $generalAndReserveAssumption->getFromDispersementOfRatesAtYearOrMonthIndex($dateIndex) :0;
    //     }
    //     return $results;
    // }
	public function getToCoverCostRates(array $dates):array
    {
        $results=[];
        $generalAndReserveAssumption = $this->generalAndReserveAssumption;
        foreach ($dates as $dateIndex=>$dateFormatted) {
            $results[$dateIndex] =$generalAndReserveAssumption ? $generalAndReserveAssumption->getToCoverCostRatesAtYearOrMonthIndex($dateIndex) :0;
        }
        return $results;
    }public function getMinCashBalancesFormatted(array $dates):array
    {
        $results=[];
        $generalAndReserveAssumption = $this->generalAndReserveAssumption;
        foreach ($dates as $dateIndex=>$dateFormatted) {
            $results[$dateIndex] =$generalAndReserveAssumption ? $generalAndReserveAssumption->getMinCashBalancesAtYearOrMonthIndex($dateIndex) :0;
        }
        return $results;
    }
	
    // public function getBankLendingRateTypeForRevenueStream(string $revenueStreamType):string
    // {
    //     return [
    //         self::LEASING => GeneralAndReserveAssumption::LEASING_BANK_LENDING_MARGIN_RATE,
    //         self::DIRECT_FACTORING => GeneralAndReserveAssumption::FACTORING_BANK_LENDING_MARGIN_RATE,
    //         self::REVERSE_FACTORING => GeneralAndReserveAssumption::FACTORING_BANK_LENDING_MARGIN_RATE,
    //         self::IJARA => GeneralAndReserveAssumption::MORTGAGE_BANK_LENDING_MARGIN_RATE,
    //         self::PORTFOLIO_MORTGAGE => GeneralAndReserveAssumption::MORTGAGE_BANK_LENDING_MARGIN_RATE,
    //         self::CONSUMER_FINANCE => GeneralAndReserveAssumption::CONSUMER_BANK_LENDING_MARGIN_RATE,
            
    //     ][$revenueStreamType];
    // }
    public function getEmployeeProfitShareRatesFormatted(array $years)
    {
        $result = [];
        $generalAndReserveAssumption = $this->generalAndReserveAssumption;
        foreach ($years as $yearIndex => $yearFormatted) {
            $result[$yearIndex] = $generalAndReserveAssumption ? $generalAndReserveAssumption->getEmployeeProfitShareRatesAtYearIndex($yearIndex):0;
        }
        return $result;
    }
    public function getBorderOfDirectorsProfitShareRatesFormatted(array $years)
    {
        $result = [];
        $generalAndReserveAssumption = $this->generalAndReserveAssumption;
        foreach ($years as $yearIndex => $yearFormatted) {
            $result[$yearIndex] = $generalAndReserveAssumption ? $generalAndReserveAssumption->getBorderOfDirectorsProfitShareRateAtYearIndex($yearIndex):0;
        }
        return $result;
    }public function getShareholdersFirstDividendPortionsFormatted(array $years)
    {
        $result = [];
        $generalAndReserveAssumption = $this->generalAndReserveAssumption;
        foreach ($years as $yearIndex => $yearFormatted) {
            $result[$yearIndex] = $generalAndReserveAssumption ? $generalAndReserveAssumption->getShareholderFirstDividendPortionAtYearIndex($yearIndex):0;
        }
        return $result;
    }public function getShareholdersDividendPaymentRatiosFormatted(array $years)
    {
        $result = [];
        $generalAndReserveAssumption = $this->generalAndReserveAssumption;
        foreach ($years as $yearIndex => $yearFormatted) {
            $result[$yearIndex] = $generalAndReserveAssumption ? $generalAndReserveAssumption->getShareholderDividendPayoutRatioAtYearIndex($yearIndex):0;
        }
        return $result;
    }
	// public function getShareholdersDividedInCashOrSharesFormatted(array $years)
    // {
    //     $result = [];
    //     $generalAndReserveAssumption = $this->generalAndReserveAssumption;
    //     foreach ($years as $yearIndex => $yearFormatted) {
    //         $result[$yearIndex] = $generalAndReserveAssumption ? $generalAndReserveAssumption->getShareholderDividendInCashOrSharesAtYear($yearIndex):'in_cash';
    //     }
    //     return $result;
    // }
    public function getFfeFormatted(array $dates)
    {
        $fixedAssetType = FixedAsset::FFE ;
        $fixedAssets = $this->fixedAssets()->where('type', $fixedAssetType)->with('fixedAssetName')->get();
        $result = [];
        foreach (count($fixedAssets) ? $fixedAssets : [null] as $fixedAsset) {
            $result[]=FixedAsset::generateFFERow($fixedAsset, $dates);
        }
        return $result;
    }
	// public function getNewBranchFormatted()
    // {
    //     $fixedAssetType = FixedAsset::NEW_BRANCH ;
    //     $fixedAssets = $this->fixedAssets()->where('type', $fixedAssetType)->with('fixedAssetName')->get();
    //     $result = [];
    //     foreach (count($fixedAssets) ? $fixedAssets : [null] as $fixedAsset) {
    //         $result[]=FixedAsset::generateNewBranchRow($fixedAsset);
    //     }
    //     return $result;
    // }
	 public function getPerEmployeeFormatted(array $positionPerDepartments)
    {
        $fixedAssetType = FixedAsset::PER_EMPLOYEE ;
        $fixedAssets = $this->fixedAssets()->where('type', $fixedAssetType)->with('fixedAssetName')->get();
        $result = [];
        foreach (count($fixedAssets) ? $fixedAssets : [null] as $fixedAsset) {
            $result[]=FixedAsset::generatePerEmployeeRow($fixedAsset, $positionPerDepartments);
        }
        return $result;
    }
    public function getGeneralFixedAssetsFundingStructureFormatted(string $fixedAssetType)
    {
        $fundingStructure = $this->generalFixedAssetsFundingStructure;
        $fundingStructureCounts = $this->getFixedAssetsWithCountsDates($fixedAssetType);
        $equityFundingRates = [];
        $equityFundingValues = [];
        ...
        foreach ($fundingStructureCounts as $monthIndex) {
            $equityFundingRates[$monthIndex] = $fundingStructure ? $fundingStructure->getEquityFundingRatesAtMonthIndex($monthIndex) : 0;
            $equityFundingValues[$monthIndex] = $fundingStructure ? $fundingStructure->getEquityFundingValuesAtMonthIndex($monthIndex) : 0;
            $newLoansFundingRates[$monthIndex] = $fundingStructure ? $fundingStructure->getNewLoansFundingRatesAtMonthIndex($monthIndex) : 0;
            $newLoansFundingValues[$monthIndex] = $fundingStructure ? $fundingStructure->getNewLoansFundingValuesAtMonthIndex($monthIndex) : 0;
            $tenors[$monthIndex] = $fundingStructure ? $fundingStructure->getTenorsAtMonthIndex($monthIndex) : 12;
            $gracePeriods[$monthIndex] = $fundingStructure ? $fundingStructure->getGracePeriodAtMonthIndex($monthIndex) : 0;
            $interestRates[$monthIndex] = $fundingStructure ? $fundingStructure->getInterestRateAtMonthIndex($monthIndex) : 0;
            $installmentIntervals[$monthIndex] = $fundingStructure ? $fundingStructure->getInstallmentIntervalAtMonthIndex($monthIndex) : 'monthly';
        }
        return [
            'fixed_asset_type'=>$fixedAssetType,
            'is_fully_funded_though_equity'=>$fundingStructure ? (int)$fundingStructure->is_fully_funded_though_equity:1,
            'equity_funding_rates'=>(object)$equityFundingRates,
            'equity_funding_values'=>(object)$equityFundingValues,
            'new_loans_funding_rates'=>(object)$newLoansFundingRates,
            'new_loans_funding_values'=>(object)$newLoansFundingValues,
            'tenors'=>(object)$tenors,
            'grace_periods'=>(object)$gracePeriods,
            'interest_rates'=>(object)$interestRates,
            'installment_intervals'=>(object)$installmentIntervals,
        ];

    }
	public function prepareGeneralFixedAssetToBeStored(array $fixedAssets){
	...
	// public function prepareNewBranchFixedAssetToBeStored(array $fixedAssets){
	// 	foreach ($fixedAssets as $index => &$fixedAssetArr) {
    //         $fixedAssetArr['ffe_item_cost'] = number_unformat($fixedAssetArr['ffe_item_cost']);
	// 		$fixedAssetArr['due_days'] = array_unique($fixedAssetArr['due_days']??[]);
    //         foreach ($fixedAssetArr['due_days']??[] as $index => $dueDay) {
	...
	// 	return $fixedAssets;
	// }
	public function prepareFixedAssetPerEmployeesToBeStored(array $fixedAssets):array 
	{
		$study = $this;
				$dateWithDateIndex = $study->getDateWithDateIndex();
	...
	protected function calculateMinCash():array 
	{
		$generalAssumption = $this->generalAndReserveAssumption;
		if(!$generalAssumption){
			return [];
			}
			// $fromDispersementOf = $generalAssumption->getFromDispersementOf();
			// $fromDispersementOfRates = $generalAssumption->getFromDispersementOfRates();
			//  $resultArr = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('revenue_contracts')
            // ->where('study_id', $this->id)
            // ->pluck('monthly_loan_amounts')->map(function ($item) {
            //     return (array)json_decode($item);
            // })->toArray();
			$sumKeys = array_keys($this->getStudyDates());
			$totalDispersement = [];
			// $totalContractAmounts = HArr::sumAtDates($resultArr,$sumKeys);
			// $this->calculateTotalDispersementFrom($sumKeys,[] , 0, $totalContractAmounts , $totalDispersement);
			
			$toCoverCostOf = $generalAssumption->getToCoverCostOf();
			$toCoverCostOfRates = $generalAssumption->getToCostCostRates();
			$totalExpenses = $this->getTotalExpenses();
			$this->calculateTotalDispersementFrom($sumKeys,$toCoverCostOfRates , $toCoverCostOf , $totalExpenses , $totalDispersement);
			$minCashBalances = $generalAssumption->getMinCashBalances();
			return HArr::sumAtDates([$totalDispersement,$minCashBalances],$sumKeys);
			
	}
	protected function calculateTotalDispersementFrom(array $sumKeys,array $fromDispersementOfRates = [] , int $fromDispersementOf = 0, array $totalRevenues = [] , array &$totalDispersement = [])
	{
		foreach($sumKeys as $dateAsIndex ){
				$rate = $fromDispersementOfRates[$dateAsIndex]??0;
				$rate = $rate /100 ;
				$sliceTotal = 0 ;
				for($i = $dateAsIndex+1 ; $i <= $dateAsIndex+$fromDispersementOf ; $i++){
					$sliceTotal+= $totalRevenues[$i]??0;
				}
				$currentSliceTotal = $sliceTotal*$rate ;
				$totalDispersement[$dateAsIndex] = isset($totalDispersement[$dateAsIndex]) ? $totalDispersement[$dateAsIndex] + $currentSliceTotal :$currentSliceTotal;
			}
	}
	...
	public function getTotalExpenses():array 
	{
			$yearWithItsIndexes = $this->getOperationDurationPerYearFromIndexes();
			        $monthsWithItsYear = $this->getMonthsWithItsYear($yearWithItsIndexes) ;
				$totalExpenses = [];
        $expenses = DB::connection(PROPERTY_MANAGEMENT_CONNECTION_NAME)->table('expenses')->where('study_id',$this->id)->join('expense_names', 'expense_names.id', '=', 'expenses.expense_name_id')->selectRaw('expenses.expense_category,expense_names.name as name,expenses.relation_name,expenses.monthly_repeating_amounts,expenses.total_after_vat,payload')->where('expenses.model_id', $this->id)->where('expenses.model_name', 'Study')->get()->toArray();
        $columnPerTypes = Expense::getColumnMapping();
        $salaryExpensesForCategories = Manpower::getSalaryExpensesPerCategory($monthsWithItsYear, $this->id, $this->company->id);
        foreach ($salaryExpensesForCategories as $manpowerCategory => $salaryExpensesForCategory) {
            foreach ($salaryExpensesForCategory as $monthIndex => $value) {
				...
				$totalExpenses[$monthIndex] = isset($totalExpenses[$monthIndex]) ? $totalExpenses[$monthIndex] + $currentValue : $currentValue;
            }
        }
		...
		foreach ($expenses as $expense) {
            ...
            foreach ($yearWithItsIndexes as $yearIndex => $monthIndexWithActive) {
                foreach ($monthIndexWithActive as $monthIndex=> $isActiveIndex) {
					...
					$totalExpenses[$monthIndex] = isset($totalExpenses[$monthIndex]) ? $totalExpenses[$monthIndex] + $currentMonthlyExpenseValue : $currentMonthlyExpenseValue; 
                }
	...
	public function getOccupiedPropertiesWithFullRentCoverageDuration(string $studyEndDateAsString)
	{
		$studyEndDateAsString = Carbon::make($studyEndDateAsString)->endOfMonth()->format('Y-m-d');
		
		$occupiedProperties = Property::
		where('company_id',$this->company->id)

		->whereHas('contracts', function ($q) use ($studyEndDateAsString) {
			$q->whereDate('contract_end_date', '>=', $studyEndDateAsString);
		})
		->with(['contracts' => function ($q) use ($studyEndDateAsString) {
			$q->whereDate('contract_end_date', '>=', $studyEndDateAsString);
		}])
		->get();
		...
		foreach(count($occupiedProperties) ? $occupiedProperties : [] as $index=>$occupiedProperty){
			...
			$formattedResult[$index]['name']=$occupiedProperty->getName() ;
			$formattedResult[$index]['area']=$occupiedProperty->getArea() ;
			$formattedResult[$index]['category_name'] = $occupiedProperty->getCategoryName() ;
			$formattedResult[$index]['type_name'] = $occupiedProperty->getTypeName() ;
			$formattedResult[$index]['due_installment'] = $occupiedProperty->dueInstallment? $occupiedProperty->dueInstallment->getTotalDueInstallmentFormatted($this->getStudyDates()) : [];
			
			
			$latestContract = $occupiedProperty->contracts->last();
			/**
			 * @var Contract $latestContract
			 */
			$dates = $this->getStudyDates();
			$formattedResult[$index]['contract']= $latestContract->formatForView($dates);
	...
	public function getOccupiedPropertiesWithPartialRentCoverageDuration(string $studyEndDateAsString)
	{
		$studyEndDateAsString = Carbon::make($studyEndDateAsString)->endOfMonth()->format('Y-m-d');
		/**
		 * ! عايزين نزود
		 * ! لو ملهاش كونتراكت وملهاش انستولمنت
		 * تيجي في الصفحه التانيه 
		 * 
		 */
		$partialProperties = Property::where('company_id', $this->company->id)->whereHas('contracts', function ($query) use ($studyEndDateAsString) {
			$query->whereDate('contract_end_date', '<', $studyEndDateAsString)
			// ->whereDate('contract_end_date', '>=', $studyStartDateAsString)
			;
			;
		})
		->with(['contracts' => function ($query) use ($studyEndDateAsString) {
			$query->whereDate('contract_end_date', '<', $studyEndDateAsString)
			// ->whereDate('contract_end_date', '>=', $studyStartDateAsString)
			;
		}])
		...
		$formattedResult = [];
		foreach(count($partialProperties) ? $partialProperties : [] as $index=>$partialProperty){
			
			$formattedResult[$index]['id']=$partialProperty->id ;
			$formattedResult[$index]['name']=$partialProperty->getName() ;
			$formattedResult[$index]['area']=$partialProperty->getArea() ;
			$formattedResult[$index]['category_name'] = $partialProperty->getCategoryName() ;
			$formattedResult[$index]['type_name'] = $partialProperty->getTypeName() ;
			$formattedResult[$index]['due_installment'] = $partialProperty->dueInstallment? $partialProperty->dueInstallment->getTotalDueInstallmentFormatted($this->getStudyDates()) : [];
		
			
			$latestContract = $partialProperty->contracts->last() ;
			/**
			 * @var Contract $latestContract
			 */
		
			$formattedResult[$index]['contract']= [
				'id'=>$latestContract->id,
				'revenue_contract'=>$latestContract->getRevenueContractWithInDate($this->getStudyDates(),$this->company),
				'collection_contract'=>$latestContract->getCollectionContractWithInDate($this->getStudyDates(),$this->company),
				'tenant_name'=>$latestContract->getTenantName(),
				'contract_start_date'=>Carbon::make($latestContract->contract_start_date)->format('d-m-Y'),
				'contract_end_date'=>Carbon::make($latestContract->contract_end_date)->format('d-m-Y'),
				'monthly_rent'=>$latestContract->getMonthlyRent(),
				'collection_interval'=>$latestContract->getCollectionInterval()
			];
			
			$propertyContractPartialRentRenewalsForProperty = $this->propertyContractPartialRentRenewalsForProperty($partialProperty->id)->first();
		
			$formattedResult[$index]['propertyContractPartialRentRenewal'] =$propertyContractPartialRentRenewalsForProperty ? $propertyContractPartialRentRenewalsForProperty->toArray() : [
				'renewal_type'=>1,
				'renovate_duration'=>0,
				'renovate_cost'=>0,
				'renewal_increase_rate'=>0,
				'renewal_duration'=>0,
				'new_rent_amount'=>0,
				'renewal_annual_increase'=>0,
	...
	public function getPropertiesToBeDelivered(string $studyStartDateAsString,string $studyEndDateAsString)
	{
		$studyEndDateAsString = Carbon::make($studyEndDateAsString)->endOfMonth()->format('Y-m-d');
		
		$occupiedProperties = Property::where('company_id', $this->company->id)->whereHas('dueInstallment', function ($query) use ($studyStartDateAsString,$studyEndDateAsString) {
			$query->whereDate('delivery_date', '<', $studyEndDateAsString)
			->whereDate('delivery_date', '>=', $studyStartDateAsString);
			;
		})
		->with(['dueInstallment' => function ($query) use ($studyStartDateAsString,$studyEndDateAsString) {
			$query->whereDate('delivery_date', '<', $studyEndDateAsString)
			->whereDate('delivery_date', '>=', $studyStartDateAsString)
			;
		}])
		...
		foreach(count($occupiedProperties) ? $occupiedProperties : [] as $index=>$occupiedProperty){
			...
			$formattedResult[$index]['type_name'] = $occupiedProperty->getTypeName() ;
			$formattedResult[$index]['due_installment'] = $occupiedProperty->dueInstallment? $occupiedProperty->dueInstallment->getTotalDueInstallmentFormatted($this->getStudyDates()) : [];
			$formattedResult[$index]['due_installment_delivery_date'] = $occupiedProperty->dueInstallment? $occupiedProperty->dueInstallment->getDeliveryDate() : '';
			
			
			$propertyToBeDelivered = $this->propertyToBeDeliveredForProperty($occupiedProperty->id)->first();
		
			$formattedResult[$index]['propertyToBeDelivered'] =$propertyToBeDelivered ? $propertyToBeDelivered->toArray() : [
	...
	{
		return $this->hasMany(PropertyContractPartialRentRenewal::class,'study_id','id');
	}
	public function propertyContractPartialRentRenewalsForProperty(int $propertyId):HasMany
	{
		return $this->propertyContractPartialRentRenewals()->where('property_id',$propertyId);
	}
	public function propertyContractFullRentRenewals():HasMany
	{
		return $this->hasMany(PropertyContractFullRentRenewal::class,'study_id','id');
	}
	public function propertyContractFullRentRenewalsForProperty(int $propertyId):HasMany
	{
		return $this->propertyContractFullRentRenewals()->where('property_id',$propertyId);
	}
	public function propertyToBeDelivered():HasMany
	{
		return $this->hasMany(PropertyToBeDelivered::class,'study_id','id');
	}
	public function propertyToBeDeliveredForProperty(int $propertyId):HasMany
	{
		return $this->propertyToBeDelivered()->where('property_id',$propertyId);
	}
	public function forecastedProperties():HasMany
	{
		return $this->hasMany(ForecastedProperty::class,'study_id','id')->with(['type']);
	}
	
	
	public function removeForecastedProperties():void
	{
		foreach($this->forecastedProperties as $forecastedProperty){
		
			$forecastedProperty->forecastedDueInstallment ? $forecastedProperty->forecastedDueInstallment()->delete() : null;
			$forecastedProperty->delete();
		}
	}
	
	public  function storeInIncomeStatementReport( array $data):void
	{
		$incomeStatementReport = $this->incomeStatementReport;
		if(!$incomeStatementReport){
			$incomeStatementReport = new IncomeStatementReport();
			$incomeStatementReport->study_id = $this->id;
			$incomeStatementReport->company_id = $this->company->id;
			$incomeStatementReport->save();
		}
		$incomeStatementReport->update($data);
	}
	public  function storeInCashFlowStatementReport( array $data):void
	{
		$cashflowStatementReport = $this->cashflowStatementReport;
		if(!$cashflowStatementReport){
			$cashflowStatementReport = new CashflowStatementReport();
			$cashflowStatementReport->study_id = $this->id;
			$cashflowStatementReport->company_id = $this->company->id;
			$cashflowStatementReport->save();
		}
		$cashflowStatementReport->update($data);
	}
	public function recalculateDueInstallments()
	{
		$rows =  PropertyContractFullRentRenewal::where('study_id',$this->id)->with(['property'])->get();
		$totalDueInstallments = [];
		foreach($rows as $row){
			$currentTotalDueInstallments = $row->property->dueInstallment ? (array)$row->property->dueInstallment->total_due_installments : [];
			$totalDueInstallments = HArr::sumAtDates([$totalDueInstallments,$currentTotalDueInstallments], $this->getStudyDates());
		}
		$rows =  PropertyContractPartialRentRenewal::where('study_id',$this->id)->with(['property'])->get();
		foreach($rows as $row){
			$currentTotalDueInstallments = $row->property->dueInstallment ? (array)$row->property->dueInstallment->total_due_installments : [];
			$totalDueInstallments = HArr::sumAtDates([$totalDueInstallments,$currentTotalDueInstallments], $this->getStudyDates());
		}
		
		$rows =  PropertyToBeDelivered::where('study_id',$this->id)->with(['property'])->get();
		foreach($rows as $row){
			$currentTotalDueInstallments = $row->property->dueInstallment ? (array)$row->property->dueInstallment->total_due_installments : [];
			$totalDueInstallments = HArr::sumAtDates([$totalDueInstallments,$currentTotalDueInstallments], $this->getStudyDates());
		}
		$this->cashflowStatementReport->update([
			'existing_properties_installments'=>json_encode($totalDueInstallments)
		]);
		
		return $totalDueInstallments;
	}
	protected  function getRevenueStreamCategoryColumnsFor()
	{
		return [
	...
	{
		$studyEndDate = $this->getStudyEndDate();
		$fullProperties = $this->getOccupiedPropertiesWithFullRentCoverageDuration($studyEndDate);
		$fullProperties=array_map(function($element){
			return ['id'=>$this->getPropertyIdentifier($element['id'],Property::FULL_COVERAGE),'title'=>$element['name']];
		},$fullProperties);
		
		$partialProperties = $this->getOccupiedPropertiesWithPartialRentCoverageDuration($studyEndDate);
		$partialProperties =array_map(function($element){
			return ['id'=>$this->getPropertyIdentifier($element['id'],Property::PARTIAL_COVERAGE),'title'=>$element['name']];
		},$partialProperties);
		
		$toBeDeliveredProperties = $this->getPropertiesToBeDelivered($this->getStudyStartDate(),$studyEndDate);
		$toBeDeliveredProperties =array_map(function($element){
			return ['id'=>$this->getPropertyIdentifier($element['id'],Property::TO_BE_DELIVERED),'title'=>$element['name']];
		},$toBeDeliveredProperties);
		
		$forecastedProperties = $this->forecastedProperties->toArray();
		$forecastedProperties =array_map(function($element){
			$typeName = $element['type']['name'];
			$area = $element['area'];
			$title = $typeName . ' - ' . $area . ' ' . __('sqm');
			return ['id'=>$this->getPropertyIdentifier($element['id'],Property::PROPERTY_FORECASTED),'title'=>$title];
		},$forecastedProperties);
		
		
		return [
			[
				'id'=>Property::FULL_COVERAGE,
				'title'=>__('Full Coverage'),
				'categories'=>$fullProperties
			],
			[
				'id'=>Property::PARTIAL_COVERAGE,
				'title'=>__('Partial Coverage'),
				'categories'=>$partialProperties
			],
			[
				'id'=>Property::TO_BE_DELIVERED,
				'title'=>__('To Be Delivered'),
				'categories'=>$toBeDeliveredProperties
			],
			[
				'id'=>Property::PROPERTY_FORECASTED,
				'title'=>__('Property Forecasted'),
				'categories'=>$forecastedProperties
			]
		];
	}
	public function areaProperties():HasMany
	{
		return $this->hasMany(AreaProperty::class,'study_id','id');
	}
	public function getPropertyIdentifier(int $propertyId,string $revenueStreamType):string
	{
		return $propertyId . '-' . $revenueStreamType;
	}
	public function createNewAreaProperty(int $propertyId,string $revenueStreamType,float $area):void
	{
		$this->areaProperties()->create([
			'property_id'=>$propertyId,
			'revenue_stream_type'=>$revenueStreamType,
			'property_identifier'=>$this->getPropertyIdentifier($propertyId,$revenueStreamType),
			'study_id'=>$this->id,
			'company_id'=>$this->company->id,
			'area'=>$area
		]);
	}
	public function removeAreaPropertiesForRevenueStreamType(string $revenueStreamType):void
	{
		$this->areaProperties()->where('revenue_stream_type',$revenueStreamType)->delete();
	}
}
